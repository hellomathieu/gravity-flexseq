...
MUTANTS = [
    ...
    ("cpp: a length of one returns the local step instead of zero (P31, may be equivalent)",
     "src/domain/LengthCv.cpp",
     "    if (effectiveLength == 0) {",
     "    if (effectiveLength == 1) {\n        return localStep;\n    }\n    if (effectiveLength == 0) {",
     "cpp-lengthcv"),
    ("ts: a length of one returns the local step instead of zero (P31, may be equivalent)",
     "sim/src/domain/LengthCv.ts",
     "  if (effectiveLength <= 0) return 0;",
     "  if (effectiveLength === 1) return localStep;\n  if (effectiveLength <= 0) return 0;",
     "ts-lengthcv"),
