...
| 4 | `Encoder` has no debounce at all, and the code says so: `_rotate_change()` carries `// Validation (TODO: add debounce check)`. Observed on hardware 2026-08-22: two fast detents produce **no event**, because a bounced detent gives +1 then −1 and the position returns to where it was | hardware, `docs/open-risks.md` line 20 | an enhancement, not a one-liner |
