...
**Last review: 2026-09-04.** Forty-five open lines, forty-eight closed or accepted.
...
| 92 | **The gesture probe assumed the depth between a screen and the tab bar, and lot 12 added a level.** `alignTab()` only rotates: it requires the caller to be on the bar already, and every course returned there with ONE long press. From the `CONFIG PATTERN` page that reaches the tab, not the bar, so the next `alignTab()` turned the FIELD cursor instead of the tab. Measured: R1 read tab 1 where it aligned to tab 2 | **FIXED on 2026-09-04.** `backToBar()` sends three long presses, which is idempotent — a long press on the bar is a no-op, `handleTabBar` having no `LONG_PRESS` case — so the harness stops assuming a depth it does not control. **The exchange is measured and it is a net gain**: green criteria 69 → 81, criteria left undecidable downstream 30 → 18, defects 4 → 5. The two that appeared, R13 and R2, were **masked** before, not newly broken | **three criteria remain, and they are named rather than guessed.** The `SUBDIV` edits of three courses read an unchanged step: the FIRST burst applies and the following ones do not. `shiftRotate` was read and it emits no detent while SHIFT is released, so that hypothesis is refuted; the swallowed first detent of the encoder (upstream defect 3) is the next candidate and is **not** established. R11's blind descending leg is still line 87, and its repair is known: give the leg a target INSIDE the list. I stopped here rather than keep guessing | **three criteria remain, and the measurement of 2026-09-04 sharpens them.** The `SUBDIV` edits of three courses read an unchanged step. ⚠️ **The account 'the first burst applies and the following ones do not' is REFUTED by the staged descent of R11**: a burst of 7 detents applied **one**, and the single detent that followed applied **one** as well — 96 → 48 → 32 ticks, so index 8 → 7 → 6. The rig walk of the same run applies its whole burst of 7 (96 → 6 ticks), so the two paths differ and the cause is **not** established. `shiftRotate` was read and it emits no detent while SHIFT is released, so that hypothesis is refuted; the swallowed first detent of the encoder (upstream defect 3) is the next candidate and is **not** established. R11's blind descending leg is CLOSED, line 87 
