...
**Last review: 2026-09-05.** Forty-seven open lines, forty-nine closed or accepted.
...
Forty-seven lines, criteria of the gesture probe count the inked slots of the tab band, and that number cannot tell the bar from the inside of a tab.** The screen model draws the tab bar without any condition, so the bar, the tab level and the `CONFIG PATTERN` page all ink 8 slots. Only the `EDIT` screen inks fewer, 3 of 8, measured on the panel. `rC_retour_barre` of R2 therefore printed `creneaux 8` while the course sat one level too deep, and it read as a success | **medium, and it is a defect of the tool.** It is a criterion that cannot fail on the distinction it claims to make. It hid the R2 defect, and it would hide the next one of the same family. The number is right; what it proves is not what the criterion says | **read the cursor instead of the slots.** Since 2026-09-05 every channel tab draws the three lines of the original, so the cursor of a field inverts its label and the bar inverts none. That reading was tried on 2026-09-05 and it was unreliable **because a SEQ tab still drew the four-field layout**; that reason is gone. The owner decides when |
