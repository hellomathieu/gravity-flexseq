...
**Last review: 2026-09-15.** Fifty-four open lines, fifty-seven closed or accepted.
...
Fifty-four lines, and each one states **what it is waiting for and from whom**. A line
that waits for nothing from nobody no longer belongs here: it is in the table
below.

| # | Subject | Severity | What is left, and from whom |
| 111 | **A measuring tool refused correctly for nine days and nobody ran it.** `FLEXSEQ_START_IN_EDIT` puts the firmware on the `EDIT` screen so that `run-blocking-probe.sh` can time a loop pass that actually draws. It navigates to the `EDIT` field of a channel, and the owner's decision of 2026-09-05 made that field exist in `SEQ` only, the factory mode being `CLOCK`. The navigation therefore never found the field, the firmware stayed on the main screen, and the probe returned zero samples with exit 1 | **CLOSED on 2026-09-15 for the tool, and OPEN as a method rule.** The repair is one line: the flag puts the channel in `SEQ` before it navigates. Measured after it: 460 bands in 65 images, **p90 6.62 ms against a budget of 12**. ⚠️ **The probe was never wrong**: it refused, it named its cause, and it exited 1. What failed is that it belongs to no routine pass, so nothing carried its refusal to anyone | **a decision of the owner, and no lot carries it.** ⚠️ **The lesson is NOT that the guard was weak.** It is that a guard which nobody runs protects nothing, and this is the SECOND case of that shape on the same day — the `CONFIG` course of the panel held a stale field index for the same nine days, for the same reason. Two outcomes: put the probe in a routine pass, and pay its five minutes · or accept that it runs on demand, and say where that demand is written |
| 109 | **93 bytes of RAM sleep in the pinned dependency, and they could live in Flash.** Measured 2026-09-14 with `avr-nm`. u8g2 declares `u8x8_d_ssd1306_128x64_noname_init_seq` (53 B) and `u8x8_ssd1306_128x64_noname_display_info` (24 B) as plain `static const` with **no `PROGMEM`**, and `u8x8_cad_SendSequence` reads them with a direct RAM read; libGravity does the same with `KNOBDIR` (16 B) | **low today, and it is an opportunity and not a fault.** RAM is not the binding resource: 143 bytes of allowed growth remain under the reserve of 384. The line exists so the bytes are not forgotten if RAM ever binds | **a decision of the owner under the ADR 0008 charter.** Recovering them needs TWO changes in the fork, not one: the attribute on the table AND the reader that consumes it. The display bring-up is what it touches, and `tools/run-screen-dump.sh` is what would prove it. ⚠️ **Do not patch `.pio/libdeps`**: the rule of `CLAUDE.md` stands |
| 108 | **`ModulatedPatternState` has a PARTIAL TypeScript mirror, and it holds the change flag only.** ⚠️ **NARROWED A SECOND TIME on 2026-09-14 by the removal of the `PATTERNS` tab**: the template editor left the C++ side, so it is no longer a missing half — what TypeScript still lacks is the PATTERN modulation buffer and its round-robin service, and nothing else. The wording below names the editor because it was written before that removal. ⚠️ **NARROWED on 2026-09-14 by sub-step 5c of lot 16E**, which created `sim/src/domain/ModulatedPatternState.ts` with `isDirty`, `markDirty` and `clearDirty`, and nothing else. The shape and the names are those of the C++, so the rest grows there instead of being reconciled with a different shape. **What follows is the wording of the day the line opened, and every word of it still holds for the other half.** **`ModulatedPatternState` had NO TypeScript mirror AT ALL, and the gap is wider than the editor.** Opened on 2026-09-14 during sub-step 5g of lot 16E, and WIDENED the same day: the first wording named the template editor alone. A search over `sim/src/` returns nothing for `ModulatedPatternState`, `modulatedPatterns`, `editorTemplate`, `heldByEditor` or `NOT_MODULATED` — one comment aside. So the missing half is the **whole structure**: the PATTERN modulation buffer and its round-robin service, born on 2026-09-01 with lot E3.6.3, AND the template editor of lot 16E on top of it. What TypeScript carries is the **render** of the editor alone, through the `templateEditor` flag of the screen model. ⚠️ **The plan of sub-step 4c made the mirror a criterion of its end** — "the ten tests green in BOTH languages" — and the lot closed without it | **medium, and it is a rule of the project that is not held.** `CLAUDE.md` says it: the two languages must represent the same contracts, and they are not independent specifications. Here a whole part of the domain exists on one side only, so no parity can be checked on it and no TypeScript mutant can aim at any of its properties. Nothing is wrong in the firmware today | **a decision of the owner, and no lot carries it.** Three outcomes. Build the mirror, and the cost is now known to be larger than it looked: a modulation buffer, a service, an editor with two guards and a deferred write · or state that this structure is OUT of the parity perimeter, and write that where the rule lives, so the gap stops reading as an omission · or decide case by case, which is the state today without anyone having chosen it. ⚠️ **Do not read the green TypeScript suite as a proof of parity here**: it holds no assertion about any of it |
| 107 | **A signature read is not a usable check before a flash.** Measured on two separate occasions, the last on 2026-09-13. `avrdude` answered `0x1e950f` at once on the first attempt of a day, and answered `not in sync` with ten attempts on a later one — while the upload that followed succeeded in 9.5 s. The named cause is the one line 101 already carries: the firmware emits on the same serial port | **low, and it is a trap for the reader, not for the module.** A red signature says nothing about the board. Reading it as a fault would send the search to the hardware | **nothing, and that is the decision.** The check is dropped rather than repaired: an upload that hangs writes nothing, so the failure mode it guarded against is already harmless. Line 101 keeps the open question of the hang itself |
| 105 | **The canvas view of the simulator still draws the four-field channel layout that the firmware dropped on 2026-09-05.** Found on 2026-09-13 while lot 16E step 4b mirrored the `PATTERNS` tab. `drawMainScreenOled()` of `sim/src/sim/MainScreenDisplay.ts` draws `LEN`, `SUB`, `SEP` and `EDIT` on two rows, and it draws the headline on a channel tab. The firmware draws the three lines of the original through `drawLegacyChannel()`, and it draws **no** headline on a channel tab: `drawMainScreen()` guards that block with `!legacy`. ⚠️ **The pixel mirror is NOT affected**: `MainScreenPixels.ts` follows the firmware, and it is the file the panel readings are compared against | **low today, and it is not a render anyone sees.** `drawMainScreenOled()` has **no caller** outside its own file and its own test, so no screen of the simulator shows the stale layout. What is real is that about ten assertions of `MainScreenDisplay.test.ts` pin a layout the firmware no longer has: they are green and they guard nothing | **a decision of the owner, and no lot carries it.** Three outcomes, and each is cheap. Delete `drawMainScreenOled()` and the assertions that pin its layout, since nothing calls it · or bring it onto the three lines of `drawLegacyChannel()`, so the two views agree · or state that the canvas view is a separate artefact and say so in its own header. ⚠️ **Do not read the green tests as a proof of parity**: the parity of the render lives in `MainScreenPixels.ts` and in the panel readings |
| 104 | **The length byte of the modulation buffer is written by the loader and read by nobody.** Found on 2026-09-13 by reading the code. `loadTemplateIntoModulationBuffer()` fills `state.length[channel]` (`include/flexseq/Persistence.h:470`), and a search over `src/` and `include/` finds no reader. A channel whose pattern a CV modulates therefore changes the **content** it plays and keeps **its own** length | **low, and it may be exactly what the specification wants.** PRD §5.2 gives the length to the channel, and the CV pattern path is forbidden to touch `baseLength`. The risk is that six bytes of RAM look like state and carry none | **a reading of the specification, and no lot carries it.** Two outcomes, and each is cheap. Either the behaviour is intended, and the field goes, which returns 6 bytes of RAM · or the played length must follow the modulated template, and the engine gains a reader. ⚠️ **Nothing is decided here.** ⚠️ **AND THE ONE CONSUMER IT HAD IS GONE since 2026-09-14**: the template editor read that byte for its own purpose, and the removal of the `PATTERNS` tab took it away. The question this line asks is therefore back to its pure form — the field is written by the loader and read by nobody at all |
| 101 | **An upload over USB hung twice without writing one byte, and the cause is not established.** Measured 2026-09-13, on the first upload of the day. `avrdude` opened the port, sent its synchronisation, and then slept at 0.0 % of CPU for more than two minutes with no output. A real write takes **3.41 s** and prints a progress bar, so neither attempt reached the write stage: the module was never touched. A read of the signature between the two attempts answered at once, `0x1e950f`, so the link and the baud rate were correct. The third attempt and the two that followed passed at once | **medium, because it stops a flash and it looks like a defect of the module.** It is not: nothing is written, and the module keeps the firmware it had. The risk is to read the hang as a broken board or a broken build | **a measurement, and no lot carries it.** The named candidate is the one this project already recorded for its EEPROM dump: the firmware that runs on the module **emits on the same serial port**, so `avrdude` reads a stale byte as its answer and then waits for bytes that never come. `tools/run-eeprom-dump.sh` already solves that by **waiting for silence** before it uploads, with a ceiling of 45 s. The same guard in front of the upload would test the candidate and remove the class. ⚠️ **Do not read the later successes as a fix**: nothing was changed between the second attempt and the third |
| 102 | **The ISP programmer cannot be connected without soldering, and the connector that fits is the power input.** Established 2026-09-13 on the schematic and the pinned dependency. The Nano carries a 2x3 ICSP footprint at the bottom, and it is **not populated**. The 6 ISP signals are also on the normal pin rows, but those rows are soldered into the module. ⚠️ **The 2x5 connector at the bottom edge is `J10`, the Eurorack power input** — `Conn_02x05_Odd_Even`, footprint `PinHeader_2x05_P2.54mm`, and the schematic declares `+12V`, `-12V`, `+5V` and `GND`. **The 10-pin output of a USBasp is the same physical connector**, so the ribbon mates without force. With the rack powered, the rails would enter the ISP pins | **high while it is unknown, and zero once it is known.** Nothing is damaged today. The mistake is one plug away, and the two connectors cannot be told apart by shape | **a decision of the owner: solder a 2x3 header, or leave the ISP path unused.** The programmer is NOT needed to flash: the USB path works and the bootloader is intact. What it would add is a **direct read of the EEPROM**, which the bootloader cannot do and which this project worked around with a dedicated firmware. ⚠️ **Two ISP pins are shared with the module**, read in `peripherials.h` of the pinned dependency: `D11` is MOSI **and** `OUT_CH6`, `D12` is MISO **and** the SHIFT button. So SHIFT must not be held during an ISP session, and OUT 6 must be unpatched. ⚠️ **And the local `GravityHW` schematic is NOT the revision of this module**: it holds no 2x3 connector and no `J11`, while the board is silkscreened `J11`. Read it as a reference, not as this board |
| 99 | **The main screen redraws on a change of the run state, and nothing tests that it fires.** Written 2026-09-09 with step 3 of lot 16E. PRD §5.0 point 6 asks the redraw trigger to watch the run state and not only the revision counter, because a MIDI Start goes through no gesture. `src/main.cpp` now compares `engine.isRunning()` to the value of the previous pass. ⚠️ **`src/main.cpp` is compiled by no native test**, so that comparison has no oracle. What IS tested is the indicator itself: four assertions per language hold its two shapes, its two positions and its absence outside the internal clock, and six mutants hold them | **low, and it is a stale image and not a wrong sound.** The indicator would keep the shape it had until the next gesture. And the case is narrow: a `PLAY` press goes through a gesture, so it already moves the revision counter. What the new condition adds is the transport started or stopped by something that is not a gesture, on the internal clock | **a probe course, or a decision to accept.** The oracle exists in principle: a panel dump before and after a transport change driven by a pin rather than by a gesture. The gesture probe replays gestures, so it is not that probe as it stands. ⚠️ **Do not add a mutant without the oracle**: a mutant whose oracle does not exist reads as an undetected mutation, which is a false signal — the rule is already recorded for the twelve tests that lot STEP-4d put to sleep |
| 97 | ⚠️ **MEASURED ON 2026-09-14 BY TAKING LOT 22 TO THE RENDER, AND THE ESTIMATE WAS RIGHT.** The owner asked for a measurement instead of a discussion. The three render criteria of lot 22 cost **136 bytes of Flash and 2 of RAM**, against an estimate of **50 to 150**: the triangle for an inactive triplet **+96**, the digit restricted to an active step **−4**, the cadence in the screen model **+44**. ⚠️ **The estimating method is therefore NOT broken for a lot of SHAPES.** The three lots that overran by 2.7 to 12 times were lots of displayed FIELDS, and calibration A measures a field at +70 bytes. The category is what the estimate must read, not the lot number. ⚠️ **The lot was REVERTED, nothing committed**, by a decision of the owner: at 30472 bytes it passed the 99 % guard by 60, while 248 bytes stayed before the linker. **It is not the lot that is too expensive, it is the budget that is finished.** ⚠️ **RE-READ ON 2026-09-14, AND THE MARGIN IS NOW 578 BYTES to the physical limit**, the image measuring 30142 of 30720 after lot 16E step 5. A savings pass of the same day returned 80 bytes and found nothing else, so the gap between the remaining lots and the space left is no longer a forecast: it is a measurement. **The per-lot Flash estimate is unreliable for a screen lot, and the three delivered lots measure the gap.** Read on 2026-09-07 from `tools/memory-baseline`, one figure per acceptance: lot 11 was estimated at 200 to 350 bytes and cost **1178** · lot 12 at −20 to 50 and cost **600** · lot 13 at 70 to 150 and cost **404**. The estimated sum is 250 to 550 and the measured sum is **2182**, so **4.0 to 8.7 times**. ⚠️ **This is NOT a scope overrun**: lots 12 and 13 were reduced on the way, and lot 11 delivered what its criteria asked. ⚠️ **The cause is named in the estimate itself**: no screen lot appeared in the measured history, so three calibrations stood in. Calibration A measured **one displayed field** at +70 bytes, and a whole screen lot is not a sum of fields | **high, because it governs the scope decision.** The remaining work is estimated at **1990 to 4030 bytes** against **2013** available — `WORKPLAN.md`, section `RM.16`. At the bottom of that range the margin is 23 bytes. ⚠️ **The bottom of the range is therefore not a neutral case**: if this ratio holds for the screen lots that remain — 22, 17a, 16, 14 and E — it is already out of reach | **a decision of the owner, and no lot carries it.** ⚠️ **No correction factor is applied, deliberately**: three values, one of them an outlier, do not make a law, and a factor chosen here would become the specification by default. Two routes exist and neither is chosen: **re-estimate** the remaining screen lots on the measured cost of 11, 12 and 13 · or **measure first**, by taking one short screen lot to acceptance before deciding the rest. The second costs a round trip and gives a real figure. This line closes when the owner says which, or accepts the estimate as it stands |
|---|---|---|---|
...
| 85 | **The output jitter grows at the top of the tempo range, and the cause is not established.** At 300 BPM the worst deviation of an output edge from the ideal grid is **3.35 ms on a 200 ms step, so 1.68 % of a 2 % budget**. It was 3.16 % before the integer tempo arithmetic of 2026-09-03, so the change moved it. ⚠️ **It is NOT noise**: three runs before and three after give the same figure to the hundredth, because this simulation is deterministic at a fixed configuration. ⚠️ **And it is NOT the display**: removing the whole render block gives 3.42 ms, so slightly worse. At 200 BPM the same measurement reads 0.53 ms, and at 20 BPM 0.29 ms | **low, and it stays inside the budget at every tempo.** What it costs is margin at one end of the range: 84 % of the budget used at 300 BPM against 21 % at 200 | **investigate BEFORE reporting it to the author of libGravity — a decision of the owner on 2026-09-03.** Sending it upstream without knowing whether the cause is in the dependency or in FlexSeq would make the author look for a defect that may be ours. The candidates not yet separated: the tick quantisation of the 96 PPQN grid, and the fitting of the ideal grid inside the probe itself |
| 86 | **The reserve of Flash beyond the assumed 30720 bytes stays unmeasured, and the method that would GUARANTEE it is excluded.** The build assumes 2048 bytes are reserved for the bootloader while its measured content is 512. Reading the `BOOTSZ` fuse would say whether **1536 bytes** are free, and it needs a programmer: the bootloader answers `0x0` for fuses. ⚠️ **Flashing through a programmer without a bootloader at all — which would guarantee 2048 bytes — was EXCLUDED by the owner on 2026-09-03**, because it costs the update over USB and the restore path of the original firmware | **medium, and it is now the last lever that could close the worst case.** ⚠️ **Re-measured on 2026-09-07.** The margin is **2013 bytes** before the 98 % guard, and **2628** before the assumed physical limit of 30720. The estimate of the remaining work is **1990 to 4030 bytes** — `WORKPLAN.md`, section `RM.16`. The worst case is therefore short by **2017 bytes** against the guard, and by **1402** against the limit. **The 1536 of this line still closes the worst case, but only against the limit**: a flash of 32256 bytes less the 28092 in service gives **4164**, which covers 4030 with 134 bytes to spare. ⚠️ **The figures of 3429 and 1151 belonged to 2026-09-04**, and this line carried them until 2026-09-07. They compared to an estimate of 2240 to 4580 that three delivered lots have since refuted | **buy a programmer and read the fuse.** The module carries a socketed Arduino Nano, verified in the schematic, so it can be programmed on the bench. ⚠️ **The purchase guarantees KNOWING, not the bytes**: if the fuse reserves 2048 the answer is that there is nothing to take |


| 87 | **The descending leg of the R11 course of the gesture probe could not detect a wrong detent count. FIXED on 2026-09-04 by a staged descent.** `kSubdivChoices` is clamped at both ends and places `/1` at index 8, and `FIELD_SUBDIV` uses `clampIndex`. A burst of 8 detents down from index 8 lands on index 0. A burst of 12 lands on index 0 as well. The criterion `R11 : cadence x24` therefore passed for **any** burst of 8 detents or more, and it hid a miscount instead of reporting it. Written 2026-09-04, when the ascending leg was found red | **Low, it is a defect of the tool and not of the firmware, and the first item is CLOSED.** The course now descends in two stages: `n - 1` detents to an interior stop, a cadence measurement there, then one detent to `x24`. The stop is index 1, one above the floor, so a detent more or less moves the reading — 8 ticks, 6 ticks, 4 ticks. The new criterion is `R11 : palier interieur`, and its expected 6 is written in full in the script. **The witnesses are split too**: the descent no longer depends on the EDIT witness, which comes after it in the course and kept the cadence criteria silent. The default detent count is now read from `flexseq::DEFAULT_SUBDIV_INDEX` instead of a copy of 8 | ✅ **The `EDGE_SPACING_MS` lever EXISTS since 2026-09-05, so both items are done.** Its first use answered the question it was asked for, and the answer is not the one expected: widening the spacing does not remove the loss of line 44, it deepens it — at 2 ms a descent of 8 applies 2, and at 4 ms even an ascent of 8 applies 2. The default is unchanged at 1 ms, so no course moves. ⚠️ **`DI_SETTLE_MS` does NOT serve for this**: raised to 70 ms it makes the 12-detent burst of another phase exceed the 750 ms ceiling of the SHIFT hold, and the harness refuses before injection, exit 4. Measured 2026-09-04. ⚠️ **The repaired criterion goes RED at once**, and that is the repair working: it reports the gesture defect of line 92 instead of hiding it. Two counter-proofs run: the expectation moved onto the measured value turns it green and lets `R11 : cadence x24` speak (`cadence 32 au lieu de 4`), and `R11_CRANS_SUBDIV=1` moves the reading from 48 to 96 ticks |

| 66 | **The measured stack peak is 201 bytes against a reserve of 384, so 183 bytes of margin.** The reserve covers the peak **1.9x**. Measured 2026-09-06 after lot S3, 6/6 watched vectors entered. ⚠️ **Three figures of this line moved together, and the paragraph below is the state of 2026-08-31.** The peak was 207, the reserve was 256, and the margin was 49. The owner raised the reserve to 384 on 2026-09-04, and lot S3 returned 15 bytes of stack. **The old ratio of 1.2x is history.** It covered it 2.1x when `RAM_RESERVE = 256` was decided on 2026-08-20, on a peak of 120. Lot LCV added 4 bytes: `advance()` now reaches `latestCalibrated()` and the quantiser, two call levels deeper than before. ⚠️ **Nothing is exceeded, and the absolute condition holds widely** — 207 bytes against 691 free. Measured 2026-08-31 by `tools/run-stack-probe.sh`, 6/6 watched vectors entered | **low, and it is a watch item rather than a defect.** The linker never sees a stack overflow on AVR: it shows as silent corruption. This reserve is the only guard | **a rule for the next lots, not a new threshold.** Any lot that adds call depth must run `run-stack-probe.sh` BEFORE accepting its footprint, not after. Do not raise the reserve and do not lower it without a fresh measurement — the 2026-08-20 decision rested on a comfort that is now spent |
...
| 44 | **A SHIFT burst on the LENGTH field is exact from 1 to 12 detents, except a burst of 8 DOWN, which applies 6.** Re-measured 2026-09-05 over the whole range a SHIFT hold allows, `SHIFT_BURST_DETENTS = 12`, in both directions, with the lever `R13_CRANS_LENGTH` on the R13 course. Twenty-three of the twenty-four cases apply every detent they ask for. The one that does not is reproducible: three runs of three read a descent of 6 for 8 requested, and the ascent of 8 in the same runs is exact | **The figures this line carried are REFUTED, and the subject survives as one named case.** It said a burst was exact up to 6 and erratic from 7, with 3, 5, 7, 5, 1 and 4 detents applied for 7 to 12 requested. Not one of those readings reproduces. They were taken on the harness that lost its first detent after any quiet period, a defect found and fixed the same day, and they measured that loss rather than a property of the burst. ⚠️ **The controls hold**: 7 detents are exact in both directions, and so are 9, 10, 11 and 12, so the loss at 8 is neither a threshold nor a size effect. The mechanism is **not** identified, and nothing in this line names one ⚠️ **THE EIGHT-DETENT DESCENT IS CHARACTERISED on 2026-09-05, and three candidate causes are REFUTED.** It is **not the detent count**: the same descent of 8 is exact from a base length of 20, and a descent of 12 from base 16 loses exactly the same two. It is **not the LENGTH value**: reaching 24 from a base of 20 by an ascent of 4 descends exactly to 16, while reaching 24 from a base of 16 by an ascent of 8 stops at 18. It is **not the direction**: the ascent of the same run is exact. The failing configuration is one combination — rig base 16, ascent 8, descent of 7 or more — and it reproduces three runs out of three. ⚠️ **Widening the edge spacing makes it WORSE, not better**: at 2 ms the descent applies 2 of 8, and at 4 ms even the ascent applies 2 of 8. What is left is a **phase coincidence** between the injected edges and what the firmware is doing, and the mechanism is **not** identified | **`LENGTH_BURST_LIMIT` IS 7 since 2026-09-05, by a decision of the owner**, on the sweep above: 7 is exact in both directions, and 8 is the only value that loses detents. The test that guards it writes 7 and 8 in full, and it refuses a burst of 8 with a binding limit of 7. ⚠️ **The eight-detent descent has its account now, and it is a sensitivity of the INJECTION, not a property of the gesture.** Nothing above 7 should be leaned on until the mechanism is named. The lever is versioned, so the sweep is replayable: `R13_CRANS_LENGTH=<1..12>` on the `recettesB` phase, and the applied count is read from the period of OUT3, `periode / 96 - 16` | ✅ **CONFIRMED on 2026-09-13 by the owner: the limit stays at 7, and the mechanism will not be chased.** Three reasons, and the first decides: the loss is a sensitivity of the INJECTION and not a property of the gesture, so no user of the module meets it · the workaround is measured, tested and it costs nothing · and the same half day spent on the remaining screen lots is worth more, because the Flash budget is decided there. ⚠️ **The reserve of this line stays**: nothing above 7 is leaned on. The sweep stays replayable with `R13_CRANS_LENGTH=<1..12>` the day somebody wants the mechanism.
...
| 91 | **The stack probe UNDER-REPORTED, and the cause is found: it scanned from the top and stopped at the first intact run of 8 bytes.** A frame allocated but not entirely written leaves such a hole INSIDE the stack region, and the scan stopped there. Measured 2026-09-04 on a SEQ image: it reported **26 bytes** where the lowest dirty byte was at 233 — one hole of **20 bytes** did it. Three of the four courses were affected, and the reported figure was lower than the true one, never higher. **The corrected peak is 233 bytes**, and the true figures are: nominal 220, nominal without injection 198, CLOCK image 220, CLOCK image with the CV reset 222, and any SEQ image **233** . ⚠️ **The peak has moved since, and 233 is the figure of 2026-09-04.** Lot S3 returned 15 bytes of stack, and the current peak is **201 bytes**, measured 2026-09-06. Read 233 as the evidence of the probe defect, never as the peak of the current firmware | **HIGH while it lasted, and it is FIXED.** The probe now scans UPWARD for the lowest dirty byte. The risk that made the downward scan attractive — an allocation at the bottom of free RAM looking like a deep stack — becomes a **named condition** instead of a silent bias: if the byte just above `_end` is dirty, the two cannot be separated and the measurement is NOT EVALUABLE, exit 1. Counter-proof run. The holes are still counted and printed, because they explain why the old method was wrong. ⚠️ **The line that reports them is written WITHOUT indentation**: the report reader recognises an interrupt vector by two spaces then a number, a trap already recorded, and I fell into it | **two things, and the second is the owner's.** ⚠️ **Every SEQ-image stack figure in the project's history is a LOWER BOUND, not a peak** — the 207 of lot STEP and the 213 of lot 11 among them. They were not wrong about the method they used; they under-report by whatever hole existed. Do not compare a new figure against them as if they were peaks. And **the reserve of 256 now covers the peak 1.10x**, against 2.1x when it was chosen in August on a peak of 120. Raising it is cheap today — free RAM is 537 bytes — and it would restore the guard's meaning. That is a decision, not a fix  ⚠️ **DECIDED on 2026-09-04: the reserve goes from 256 to 384**, and it is a REALLOCATION and not a loosening — the same figure is the boundary between the data and the stack, so the stack gains what the data gives up. Derived from a measurement: the peak is 233, lot 11 cost **28 bytes of stack on its own** (205 → 233, measured on the pre-lot-11 firmware with the corrected probe), and five interface lots remain that add call depth — `233 + 5 × 28 = 373`, which 384 covers. The cap on static data falls from 1792 to 1664, so allowed growth falls from 281 to **153 bytes**, and the guard bites at 538 as it should. ⚠️ **That the remaining work fits in 153 bytes is REASONED, not measured**: the roadmap carries Flash estimates per lot and no RAM estimate at all |
| 92 | **The gesture probe assumed the depth between a screen and the tab bar, and lot 12 added a level.** `alignTab()` only rotates: it requires the caller to be on the bar already, and every course returned there with ONE long press. From the `CONFIG PATTERN` page that reaches the tab, not the bar, so the next `alignTab()` turned the FIELD cursor instead of the tab. Measured: R1 read tab 1 where it aligned to tab 2 | **FIXED on 2026-09-04.** `backToBar()` sends three long presses, which is idempotent — a long press on the bar is a no-op, `handleTabBar` having no `LONG_PRESS` case — so the harness stops assuming a depth it does not control. **The exchange is measured and it is a net gain**: green criteria 69 → 81, criteria left undecidable downstream 30 → 18, defects 4 → 5. The two that appeared, R13 and R2, were **masked** before, not newly broken | **CLOSED on 2026-09-05, and the cause was not the one this line named.** The account 'the first burst applies and the following ones do not' is refuted. The harness lost the FIRST detent after any quiet period, because it settled after a detent and never before one — see line 44. `rotate()` now settles before its first detent. The probe reads **103 green criteria, 2 defects and 0 undecidable** ones. ⚠️ **`R2` was the same fault at another site**, found 2026-09-05: the course left the `CONFIG` page with ONE long press, so the navigation that followed ran one level off and ended on `MOD`, the one field with no effect today. `backToBar()` fixes it, and the probe reads **104 green criteria, 1 defect and 0 undecidable** ones. ⚠️ **`R13` was the SAME fault at a third site**, found the same day, and it is fixed the same way. ✅ **The gesture probe returns PASS for the first time: 104 green criteria, 0 defect, 0 undecidable criterion, exit code 0.** Counter-proof run: `EXPECT_R13_PERIODE=1920` turns one criterion red and leaves the other green |
...
|---|---|---|---|
| 103 | **A power cut during a template edit left channel 1 in `SEQ`.** The template editor took channel 1 into `SEQ` while it was open, and gave the mode back when it closed. The periodic scan of the persistence writes the mode of every channel, so a cut between the two moments persisted the forced mode | **CLOSED on 2026-09-14, and the cause went with the feature.** The `PATTERNS` tab and its template editor are REMOVED — PRD §5.0, third amendment of 2026-09-14. Nothing forces the mode of a channel any more, so the window this line described cannot open | **nothing.** ⚠️ **The decision this line recorded is NOT retracted**: on 2026-09-13 the owner chose the audition that always works over a screen that says it is silent. That choice is simply moot, the editor no longer existing |
| 110 | **The action field offered `SAVE`, and a short press on it did nothing.** Opened 2026-09-14 with lot 16E step 5, `SAVE` being out of its scope: PRD §5.0 amendment 1bis said a short press runs the action, so the specification and the code diverged on `SAVE` alone | **CLOSED on 2026-09-14, and the field went with it.** The owner read the screen on the module and found the choice of a pattern too complicated. PRD §5.0 amendment `1ter` moves the load onto `SHIFT` plus a rotation, and the action field leaves the channel tab. `SAVE` therefore offers nothing any more, and no divergence remains | ⚠️ **The absence stays, and it is not a defect**: no interface writes a channel copy into a template, which was already true. §12.9 places that design in lot E |
| 106 | **The audition of the template editor looked broken on the module, and the cause was the power.** Reported 2026-09-13: channel 1 seemed to emit on every step instead of on the active ones, and all six channel LEDs looked lit. The module was running on the USB alone, out of the rack | **CLOSED the same day, and it was never a defect.** The owner re-tested with the module powered by the rack: output 1 follows the active steps. On the USB alone the logic runs — the screen, the encoder, the gestures — and the ±12 V rails are missing, so the output stage cannot be judged | **nothing to repair in the firmware, and the rule that covers it already existed.** `CLAUDE.md` says it: upload out of the rack, **measure in the rack**, USB unplugged. ⚠️ **What is added is the precision that was missing**: on the USB alone the screen is trustworthy and **the outputs are not**. A native test proved the domain throughout, and it was right. ⚠️ **The six LEDs are explained by the same cause**: the owner confirmed on 2026-09-13 that he saw them while the module was on the USB and not on the rack supply. Nothing of this report survives as a defect |
| 98 | **The gesture probe reported two failures that do not reproduce, and the run environment was the only candidate.** The probe reported FAIL on two criteria of its R11 course on 2026-09-09. Two runs on the same commits, in a process environment checked clean, both give PASS with 106 green criteria, 0 defect and 0 undecidable. The code was therefore never the cause | **CLOSED on 2026-09-13 by a decision of the owner: route B.** The eight probes that drive the simulator now refuse to start while another run of a harness is alive. `tools/no-stale-simavr.sh` is sourced by each one, it names the process it found, and it exits **6** — a code that no other outcome uses, so the refusal cannot be read as a defect of the firmware. `ALLOW_STALE_SIMAVR=1` lifts it for a deliberate parallel run. Verified: the eight refuse with 6 against a live harness, the lever lifts the refusal, and a run without one passes | **the residual is ACCEPTED, and it is named.** The cause of the two reds stays unknown, and route A — reproduce the red on purpose beside a live harness — was weighed and set aside: it would prove nothing on an absence. What the guard removes is the **class**, for every probe, not the one case. ⚠️ **Do not read the guard as an explanation**: it makes the environment controlled, it does not say what happened on 2026-09-09 |
| 28 | **The navigation lacked two elements of the original, and one glyph was misleading.** The original bar carries 7 tabs plus a **separate status glyph** at x = 121: `t` when stopped, `r` when playing, drawn only when the clock is internal. FlexSeq drew 8 navigable tabs, and the eighth was a filled 5x5 square that the owner read as a stop indicator on the module | **CLOSED on 2026-09-09 by steps 1 to 3 of lot 16E.** The bar holds **9** navigable tabs in slots of 12 px, the value of the original. The eighth carries a grid of steps for PATTERNS and the ninth two sliders for CONF, both 7 px by 5 px. The indicator is drawn outside the navigation, at x = 121 stopped and x = 122 running, with the two glyphs of the original's own font, on the internal clock only. Read on the panel of `env:mainscreen`, and held by four assertions per language plus six mutants. ⚠️ **One element of the original is NOT taken and it is a decision to make**: the original inverts the selected tab a second time while SHIFT is held, and FlexSeq does not react to SHIFT in the bar — that row of `docs/original-conformity.md` stays a divergence to decide |
| 61 | **`main` and `PagedScreen::renderFrom` hold about a third of the Flash in service, and their size is not a saving.** Read by `avr-nm` on the ELF of 2026-09-06: `main` **6664 bytes**, `renderFrom` **2772**, together **9436**, so **33.1 %** of the Flash in use. ⚠️ **The two exchange code, so neither can be tracked alone over time**: between 2026-09-04 and 2026-09-06 `renderFrom` returned 276 bytes while `main` took 294, the tab layout having migrated from one to the other. The figures of 5976 / 2490 and 6370 / 3048 that this line carried are superseded | ✅ **CLOSED on 2026-09-07 by the owner.** Lot S3 answered the question this line asked, and **the answer is that there is nothing more to take without giving something up**. The line stays written so that nobody reopens the two symbols on their size alone | **lot S3, and its measurement is the result.** ⚠️ **`main` is 16 bytes without inlining**: it is a gathering point, not an aggregate that would hold its own fat. Six counter-builds costed six candidate removals, and none is fat. The lot returned **426 bytes** by four steps measured one by one, and none of them came from opening either symbol. **Four zones were examined and closed**: `UiController::handle` (980 bytes) **is** the gesture map · the transport (894 bytes) has nothing to share, `Tick`, `SetTempo` and `SetSource` having one caller each · the typography, by a decision of the owner · the screen model, an uncertain gain against the risk of a torn image. ⚠️ **Two techniques of the lot do NOT work here**, so nobody retries them: the inlining report returns nothing, and disabling inlining at link time has no effect. **The compiler decides inlining at COMPILE time, in spite of `-flto`**. ⚠️ **The conclusion of the lot holds, and it is the project's open question**: the savings do not close the budget, the scope does |
| 94 | **CLOSED on 2026-09-05.** Several criteria of the gesture probe count the inked slots of the tab band, and that number cannot tell the bar from the inside of a tab.** The screen model draws the tab bar without any condition, so the bar, the tab level and the `CONFIG PATTERN` page all ink 8 slots. Only the `EDIT` screen inks fewer, 3 of 8, measured on the panel. `rC_retour_barre` of R2 therefore printed `creneaux 8` while the course sat one level too deep, and it read as a success | **FIXED, and the fix is proven by a counter-proof.** The probe publishes the highlighted line: **-1** on the bar, **0 to 2** inside a tab, **-2** when more than one line is marked. It reads the top row of the cursor box, where the inversion puts ink and nothing else does. Calibrated on four known states: bar `0/0/0`, tab `24/0/0`, `CONFIG` page `0/31/0`, bar again `0/0/0`. Three criteria now require the bar — the entry into `EDIT`, the return of R2, the composition of R13. ⚠️ **Counter-proof executed**: with the single long press put back at the two sites, R2 turns red with `curseur 2` and R13 turns undecidable for the same reason; both were green on that same gesture before. The probe reads **PASS, 104 green criteria, 0 defect** | **the repair is DONE, and this column records what it took.** Since 2026-09-05 every channel tab draws the three lines of the original, so the cursor of a field inverts its label and the bar inverts none. That reading was tried earlier the same day and it was unreliable, **because a SEQ tab still drew the four-field layout**; that reason is gone |
| 93 | **A channel tab in CLOCK or RANDOM mode has no EDIT field and no CONFIG field, so the gesture probe cannot reach the EDIT screen.** `UiController::fieldAt()` serves two maps. `isLegacyModeTab()` is true when the mode is CLOCK or RANDOM, and the three fields are then MODE, OFFSET or SUBDIV, and MOD. Only a channel in SEQ mode carries MODE, EDIT and CONFIG. The factory default is CLOCK, so the courses that use the factory image can never enter EDIT. Measured 2026-09-05: 18 criteria are undecidable across four courses | **CLOSED on 2026-09-05.** The owner settled both halves. `EDIT` from a `SEQ` channel only is **wanted**: it is a feature of `SEQ`. And a channel tab must show the pattern name large on the left with `MODE`, `EDIT` and `CONFIG` on the right — which `PRD.md` §12.1 already specified, and which the screen did not draw. The renderer now uses the layout of the original in every mode, the four-field branch is gone, and `legacyLayout` left the screen model. ⚠️ **Two earlier explanations stay REFUTED, so nobody retries them**: the third grid row of EDIT does not fill the tab band, and the navigation was not the cause | **the proof is on the panel and in both languages.** `env:mainscreen` built with `-DFLEXSEQ_DEMO_MODE_SEQ=1` inks **1003 pixels** on a `SEQ` tab, and the `TypeScript` mirror renders the same number row by row. The gesture probe sets the mode by a gesture before it asks for `EDIT` — the `MODE` field clamps on `CHANNEL_MODE_COUNT`, so a burst of that many detents lands on `SEQ` from any mode and the gesture is idempotent. Measured: it now enters `EDIT`, `creneaux 8 -> 2`, green criteria **81 to 91** and undecidable ones **18 to 8**. Four mutants guard it, two per language, each verified detected. Footprint: RAM **-15 B**, Flash **-340 B** |
| 90 | **The channel tab of the original took about 13 ms in one loop pass, against a budget of 12 ms.** Measured 2026-09-04: p90 13.02 ms, median 9.60, band 3.38 ms, frame 57.5 ms, where the EDIT screen read 6.28 ms with an identical band transfer — so the whole gap was drawing | **CLOSED on 2026-09-04 by routes 1 and 2, both chosen by the owner.** Route 1: the text widths move into `MainScreenModel` and `measureMainScreen()` fills them once per frame instead of up to six times per band. Measured **13.02 to 12.14 ms**, so 0.88 ms against the 0.91 the counter-build predicted, and the panel reads **965 pixels with zero divergent row** — the render did not move. Cost RAM +6, Flash +238. Route 2: the probe takes two regimes of redraw, `continuous` by default at 12 ms and `gesture` at 16, and the channel tab is a gesture screen because outside `EDIT` production starts a frame only when `ui.revision()` changes. **ADR 0001 carries the decision, its four counter-proofs, and two facts worth more than the closure itself**: both pass budgets are CONVENTIONS — the 12 ms has no derivation in this repository, and two of the three things it claims to bound give stricter numbers — while a DERIVED criterion does exist and now runs, the routine frame against `UI_MIN_INTERVAL_MS` read from `src/main.cpp`. It discriminates: 32.5 ms for the editor, 54.3 for the channel tab against an interval of 40. ⚠️ **The 2.11 ms of the 23-pixel glyph are NOT recovered and will not be**: that is what the typography of the original costs, and the owner chose it |
| 96 | **The SELFTEST of the gesture probe had not run since 2026-08-29, and its expectations no longer matched what the probe reported.** It replays thirty-two verdict cases and three source mutations, and each one expects a verdict, an exit code and a cause | **CLOSED on 2026-09-06. It runs green: exit 0, 79 criteria, 36 courses, 1 h 29 min.** Five passes established it. The first named four failing subjects. Three were defects of the tool and one was a single event | **Four commits, and none touches the firmware.** `ff00653` names a course that stops before its verdict a COURSE INCOMPLETE, keeps its log outside the temporary directory, and says the failure belongs to the tool — the SELFTEST read a missing verdict as a defect of the firmware. `15b0857` prints the tally of finished courses on a failed pass too. `7bb903e` repairs two cases: `R2_ROTATIONS` asked for the value it already held, and `R11_CRANS_SUBDIV=0` expected three defects where the staged descent of 2026-09-04 correctly gives an undecidable verdict; the proof that case carried moves to a new case, which shifts the list of refused codes. `9449d29` makes the R10 recipe reach the step it names. ⚠️ **The nominal pass is unchanged at PASS with 106 criteria**, so no validation of the firmware was ever wrong. ⚠️ **One course out of about 130 stopped before its verdict, once, on 2026-09-05, and it did not recur.** Its cause is unknown and is no longer examinable, because the working log was overwritten. The guard of `ff00653` names the next one and keeps its log |
| 37 | **`MAX_SKIP_CHANCE` was 10 in the code and 9 in the PRD.** PRD §16 capped the skip chance at 9 on 2026-08-23, the 90 % the original shows and the value its interface clamps to. The decision was taken and it was not applied, so the gesture offered eleven steps instead of ten | **CLOSED on 2026-09-06 by commit `5cd0fa1`.** | **The repair needed TWO bounds, and that is the part the line did not say.** One constant governed the interface and the engine at once, so lowering it to 9 would have repaired the first and broken the second: the original clamps the value the user sets to 9, and the value plus its CV modulation to 10 in the generator. `MAX_SKIP_CHANCE_SETTING` is 9 and bounds the interface; `MAX_SKIP_CHANCE` stays 10 and bounds the engine. Both languages carry the pair, both tests were seen red first with the same reading — expected 9, was 10 — and two mutants raise the setting back to 10. **Cost: zero bytes**, a bound in a clamp being an immediate value |
| 89 | **The TypeScript view drew the OLD FlexSeq layout, and no test asserted an absolute text width.** The firmware drew the three lines of the original while the preview drew a headline box of ten rows. Removing the one-pixel tracking left 595 tests green, so nothing measured a width at all | **CLOSED on 2026-09-04 by step 7 of lot 11.** Two pixel mirrors now exist — `MainScreenPixels.ts` and `PatternScreenPixels.ts` — and against the models that `env:mainscreen` and `env:wokwi` freeze they render **965** and **786** pixels of ink with **zero divergent row**. Thirty-three lines of the vector file turn from `cpp` to `both`. ⚠️ **The first render was 972 and not 965**, and the cause is a real difference of model: `u8g2_GetStrWidth` removes the advance of the last glyph and adds that glyph's ink width plus its offset, so `MOD:` measures 17 and not 18. The cursor box was one pixel too wide on each of its seven rows. `textWidth` follows u8g2 now, and `textAdvance` keeps the sum. ⚠️ **Two limits are named rather than left implicit**: the comparison is against the panel of the SIMULATION, never a physical module, and only two models carry a panel reference — `sim/mockups.html` prints, on each of its eleven screens, whether that screen has one |
| 29 | **One font for everything, so no parameter read as the main one.** FlexSeq called `setFont(u8g2_font_5x7_tr)` once and never changed it, so the pattern name and the tempo had the size of a label. The ten custom glyphs meant to replace `logisoso26` were designed and never implemented | **CLOSED on 2026-09-04 by lot 11**, and by the route the owner chose on 2026-08-23: reuse the fonts of the original rather than redraw. Both are in the firmware **byte for byte** — `velvetscreen` 437 bytes for every label, `stkL` 569 bytes for the main parameter, drawn 23 pixels tall. u8g2's own font left the production image; `env:bringup` keeps it, being a hardware diagnostic with its own budget. ⚠️ **The ten custom glyphs are therefore ABANDONED, and that is the second face of the decision**: they are not deferred, they have no purpose left. Measured cost of the whole lot: Flash +1178 B against 2026-09-03, RAM +3, stack +6. `docs/original-conformity.md` reads **conform** on the fonts row and on six rows of a channel tab |
| 88 | **`MOD` on a channel tab had no destination to point at, and two sections of the PRD said opposite things.** §12.1 gave line 3 of a CLOCK tab and of a RANDOM tab the label `MOD:`, because the original does, and there that field says which CV modulates the main parameter of the mode. §10.2 fixed the destinations to `none / PATTERN / LENGTH / RESET / STEP`, explicitly, and neither the subdivision nor the skip chance was in that list | **CLOSED on 2026-09-04 by the owner.** The destinations now **depend on the mode**: `CLOCK` modulates the subdivision and `RANDOM` the skip chance, as in the original; `SEQ` keeps the explicit choice among four, because it alone holds two modulatable parameters. PRD §10.2 is amended, and the reason its earlier paragraph gave is exactly why the rule splits. ⚠️ **The mechanism does not exist yet**: lot 11 draws the line and it reads `OFF`, which is true, and lot 13 makes it live |
| 84 | **The tempo range did not match the original firmware, and nothing said what a stored value outside a new range would do.** The interface accepted `[30, 300]` while the original accepts `[20, 200]` and the plan announced 20 to 200, which lowers the maximum by 100 as well as the minimum by 10. A tempo between 201 and 300 was storable and would have fallen outside. Found 2026-09-03 by condition 8 of lot S2.1-b | **medium, and it was a persistence question as much as a bound** | **closed 2026-09-03 by a decision of the owner: the range becomes `[20, 300]`.** Keeping 300 is a deliberate divergence from the original, of the class the project already accepted for the four external clock rates against the original's two. ⚠️ **The persistence question DISAPPEARS**: the new range contains the old one, so no stored value can fall outside. The clamp to the nearest bound is implemented anyway, as a net for a future narrowing. ⚠️ **And the bounds were guarded by NOTHING**: the tests compared against `MIN_TEMPO` and `MAX_TEMPO` themselves, so moving a bound moved the expectation with it, and no mutant aimed at them. One test per language now writes 20 and 300 in full, seen red as `Expected 30 Was 20`, and four mutants were added and detected. Measured on the pins at 20 BPM: a step of **2999.86 ms** against 3000.00 expected, jitter 0.29 ms, so 0.01 % of its budget |
| 82 | **Only ONE of the four external clock rates is guarded by the routine pass.** Lot XCLK proved the external clock at input PPQN 24, 4, 2 and 1, and on the MIDI transport. Only `extclock` at PPQN 24 joined the nominal pass, because the freeze of defect 12 makes the other three need 31, 50 and 87 seconds of simulation, against 15. The `midiclock` course is on demand for the same reason. Measured 2026-09-03 | **low, and it is a coverage debt rather than a defect.** All five courses were green on the tree of the closure, so nothing is known to be broken. What is missing is the guard against a later regression | **closed 2026-09-03** by the repair of the freeze, pin `c2ec8ab`. The four rates become four named courses — `extclock24`, `extclock4`, `extclock2`, `extclock1` — plus `midiclock`, and all five are in the routine pass. Each carries its own duration, derived from its rate: 12, 16, 20, 27 and 12 seconds. Without the freeze the total simulation falls from 183 to 87 seconds, and the whole pass of **sixteen courses runs in 41.6 seconds of wall clock**. The two problems were one problem |
| 63 | **`ASCII=1` on `run-screen-dump.sh` printed no image at all.** The script extracted the art with `sed -n '/^--- /,/^   +-*+$/p'`, and the range closed on the FIRST border line, which is the border ABOVE the panel. The observed output was two headings and two borders, and zero rows of panel. Measured 2026-08-30 during lot F | low. The verdict was unaffected: it comes from the harness's own `rotation OK` line, not from the art. What was lost is the way a reader looks at the render with their own eyes | **closed 2026-09-03**, during condition 10 of lot S2.1-b, which needs exactly that instrument. An `awk` block replaces the range: it prints from each heading and stops on the SECOND border. Measured after the fix: 128 rows of panel on the two views, and the panel of `env:nanoatmega328` reads `A1`, `LEN 16`, `SUB`, `SEP` and `EDIT` to the eye. The nominal pass is unchanged, 808 pixels of ink and its three criteria green |
...
**A defect that no native test can see belongs to the pin probe, and only the
pins find it.** Written 2026-09-14, with PRD §5.0 amendment `1ter`. The
controller cancelled its confirmation on every event that was not `SHIFT` plus a
rotation. Every native test passed, in both languages, because a native test
calls `handle()` directly. On the module the adapter emits `EVENT_SHIFT_PRESS`
at **every release of SHIFT**, and the rotation does not suppress it — only the
long press is suppressed, `src/hal/InputAdapter.cpp`. The release therefore
cancelled the question at each detent, and no detent ever confirmed: the load
never happened. ⚠️ **The screen kept redrawing**, so the gesture was seen and the
fault looked like a load that did nothing. **Two rules come out of it**: an event
that changes nothing in the controller must cancel nothing · and a gesture whose
tail produces its own event is only proved on the pins.

**To take a function out of line pays only when the compiler expands it at
SEVERAL call sites, and each candidate is measured alone.** Written 2026-09-14,
after a savings pass on the two largest symbols of the image. `main` held the
persistence codec expanded at several sites, and two `noinline` attributes
returned 80 bytes. The three candidates do not behave alike, and only a
measurement separates them: `byteAt` returns 44 bytes, `applyByte` returns 34,
and `resetToDefaults` **costs 20**, because it has a single caller and pays the
prologue for nothing. ⚠️ **`PagedScreen::renderFrom` GIVES NOTHING, and that is
measured**: its 3502 bytes are the two screen renderers, each called from ONE
place, so there is no copy to collapse; out of line they cost 36 bytes. **Do not
retry that direction.** ⚠️ **And a saving is not free because it is small**: the
last u8g2 option returns 20 bytes and costs **0.90 ms of loop pass**, 5.84 to
6.74 ms, which is 15 %. It was refused. **Two rules come out of it**: measure
each candidate on its own, never a group · and price a Flash saving in time
before taking it.
...
hash read the title, the measure separation and the two cursor flags — never the
length. The screen therefore never redrew the number while the length changed
under it, and the owner read it on the module. The first breach was the same
band on 2026-09-04, with the measure separation. Both reached the module. The
hash now lives beside a comment that names the rule, and a test holds it.

**A criterion that reads "at least one onset" does not discriminate a channel
that emits on every step.** Written 2026-09-13. A test of the audition asserted
that channel 1 emits at least once. A channel in `CLOCK` emits on every step, so
the test passed on the very behaviour it was written to exclude. The exact count
is what separates the two, and it needs a fixture whose content is known: the
first count came out at three because the template of the harness already
carried two steps of its own.

**Two readers of one decision diverge, and the screen freezes.** Written
2026-09-13. The redraw trigger asked which channel is selected; the frame
selector asked whether the template editor is open. The `PATTERNS` tab selects no
channel, so the playhead never redrew there, and the editor closing in deferred
time redrew nothing either. The two now read one function,
`uiFrameChoiceOf()` of `include/flexseq/UiFrame.h`, which lives in a header and
not in `src/main.cpp` because no native test compiles that file — line 99.
...
render as the suspect. Two runs on the same commit, in a process environment
checked clean, both give PASS with 106 green criteria. A leftover run of the simulator from an
earlier, interrupted pass is the only candidate anybody named, and it is
unproven. The rule: a probe that drives a simulator
checks that no earlier run survives before it reports, and a red that follows an
interrupted run is re-run before it is written down. A red kept without that
check sends the reader to look for the defect in the wrong place, which is the
fault this document already records.
...
index of the SUBDIV field of the CONFIG page. Lot 12 made that index 1, and the SELFTEST
case passed 1. The run was then identical to a nominal run, byte for byte: nothing was
injected, so no criterion could turn red, and the case read as a failure of the probe. The
rule: a lever whose default comes from a constant is never given the value of that
constant, and its help text names the trap.

**A harness that computes a detent count is exercised at its degenerate value.** Written
2026-09-06. The R10 recipe rotates by the difference between its target and the current
step. When the two were equal the difference was zero, and the code rotated 24 instead. The
cursor landed on step 29, the criterion stayed green for a step it never names, and the
next recipe was reported as a defect of the firmware. This is the second time the probe
named one step and visited another, and both times it ended on step 33. The rule: a
computed gesture count is exercised where it collapses, and not only at its nominal value.

**Never edit a shell script while one of its runs is in flight.** Written 2026-09-06, after
a lost measurement. Bash reads a script as it runs it, so an edit shifts the file and the
running shell reads incoherent text. The run died on a syntax error at a line the edit had
not touched, which sends the reader to look for the defect in the wrong place. The rule: a
measurement and an edit of the same script never overlap.
...
**A WATERMARK SCAN MUST LOOK FOR THE FIRST DIRTY BYTE, NEVER FOR THE FIRST CLEAN
RUN.** The stack probe painted free RAM and then scanned from the top for the
first run of 8 bytes still carrying the pattern. That run was taken as the
boundary of the stack. It is not one: a frame can be allocated without being
entirely written, which leaves an intact hole INSIDE the region the stack
reached. Measured 2026-09-04, a single hole of 20 bytes made the probe report
**26** where the true peak was **233**.

⚠️ **The direction was chosen for a real reason, and that reason survives.** An
allocation at the bottom of free RAM would dirty the pattern without the stack
having descended there, so scanning upward for the first dirty byte has its own
failure mode. The fix is not to pick the lesser evil silently: the risk becomes a
**named condition**. If the byte immediately above `_end` is dirty, an allocation
and a deep stack cannot be separated, and the measurement is declared not
evaluable rather than reported.

**The general rule: when two scan directions each have a blind spot, do not
choose one and forget the other. Make the blind spot of the chosen direction a
criterion that fails loudly.**

⚠️ **And a figure that cannot be explained must be pursued, not filed.** The 26
bytes were recorded as a signal on the same day, with the sentence "either it is
correct and the SEQ render is genuinely shallow, or the probe under-reports on
that path, which would put every SEQ-image figure in doubt". The second branch
was the true one. The cost of pursuing it was one hour; the cost of filing it
would have been a reserve believed to cover a peak it did not.

**An index is not a distance, and a tool must not spend one for the other.**
Written 2026-09-05, from line 93. The gesture probe turns the encoder by
`rotate(SEQ_FIELD_INDEX_EDIT_ENTRY, 1)`. The published index says **where** a
field sits. The rotation needs **how far** to go, which is `target - current`.
The two are equal only when the cursor starts on field 0, and the probe does not
control that. This is the same class as the depth of line 92 and the grid modulo
of line 87: a tool that assumes a state it does not own. ⚠️ **The rule holds, and
the example that produced it is REFUTED.** An absolute navigation was built on
2026-09-05 and measured: the cursor reached the field and the press still did not
enter EDIT. The cause is the field map of line 93, not the arithmetic. Read the
rule as a rule, never as the account of that defect. ⚠️ **The repair of lot
11 went half way, and that is worth knowing.** It replaced hard-coded detent
counts with the published indices, which closed the shift of the field list. It
did not turn a position into a distance, so the harness still starts counting
from a place it hopes for.

**A hypothesis about a witness is tested against the panel, not against the
geometry.** Written 2026-09-05, same line. The reading `creneaux 8 -> 8` was
explained by the three-row grid of EDIT reaching line 63 and inking the tab band.
The arithmetic supported it: the band is lines 56 to 63, and `GRID_BOTTOM_Y` is
63. The panel refuted it: an EDIT screen inks **3 of 8** tab slots, because the
discs sit on line 54 and nothing reaches line 61. The rule: a bound that two
constants share does not prove that pixels are drawn there. Read the panel.

**A settle that is applied after an event must be applied before the first one
too.** Written 2026-09-05, from lines 44 and 92. The gesture harness waited
50 ms after every detent and nothing before the first. Every course therefore
lost its first detent after any quiet period, and the gesture that followed
acted on the wrong field or on nothing. The defect hid for months because it
only bites where a course has no slack of its own: the same rotation worked in
one course and failed in another, which sent the search to the direction of the
rotation and to the size of the burst. Both were refuted by measurement. ⚠️ **The
rule is general**: a settle time states how long a system needs to be ready, so
it belongs on **both** sides of a train of events, not only between them.

**A lever is worth adding even when it refutes the hypothesis that asked for
it.** Written 2026-09-05, from lines 44 and 87. `EDGE_SPACING_MS` was asked for
so that an interrupt hypothesis could be tested. Its first use showed the
opposite of what was expected: widening the spacing deepens the loss instead of
removing it. The lever is kept, and the hypothesis is dropped. ⚠️ **The rule is
about what a lever is FOR**: it exists to make a question answerable, not to
confirm an answer. A lever that only ever agrees with its author would not have
been worth its code.
