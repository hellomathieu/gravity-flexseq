...
| 108 | **The domain of the template editor has NO TypeScript mirror.** Found on 2026-09-14 during sub-step 5g of lot 16E. `editorTemplate`, `heldByEditor`, `serviceTemplateEditor` and now `channelIsAudible` exist on the C++ side only: a search over `sim/src/` returns nothing for any of them. What TypeScript carries of the editor is the **render** alone, through the `templateEditor` flag of the screen model. ⚠️ **The plan of sub-step 4c made the mirror a criterion of its end** — "the ten tests green in BOTH languages" — and the lot closed without it | **medium, and it is a rule of the project that is not held.** `CLAUDE.md` says it: the two languages must represent the same contracts, and they are not independent specifications. Here a part of the domain exists on one side only, so no parity can be checked on it and no TypeScript mutant can aim at those properties. Nothing is wrong in the firmware today | **a decision of the owner, and no lot carries it.** Three outcomes. Build the mirror, and the cost is real: the editor is a state machine with a service, two guards and a deferred write · or state that the editor is OUT of the parity perimeter, and write that where the rule lives, so the gap stops reading as an omission · or decide case by case, which is the state today without anyone having chosen it. ⚠️ **Do not read the green TypeScript suite as a proof of parity on the editor**: it holds no assertion about it |
