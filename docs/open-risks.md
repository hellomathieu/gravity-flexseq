...
**Last review: 2026-09-04.** Forty-six open lines, forty-seven closed or accepted.
...
| 88 | **`MOD` on a channel tab has no destination to point at, and two sections of the PRD say opposite things.** PRD §12.1, amended 2026-09-04, gives line 3 of a CLOCK tab and of a RANDOM tab the label `MOD:`, because the original does. In the original that field says **which CV input modulates the main parameter of the mode** — `SUBDIVISION` in CLOCK, `SKIP CHANCE` in RANDOM (`UI.ino:134,166-172`). But PRD §10.2 fixes the destinations to `none / PATTERN / LENGTH / RESET / STEP`, **explicitly**, and it says so against the earlier design that deduced the destination from the mode. **Neither the subdivision nor the skip chance is in that list**, so a CV cannot reach either. Drawing `MOD:` on a CLOCK tab would therefore show a routing that cannot do what the label promises. Found 2026-09-04 during step 4 of lot 11 | **medium, and it blocks one line of one screen.** Nothing is wrong in the code: the destinations do what §10.2 says, and the routing is persisted and tested. What is wrong is that two sections of the PRD describe the same field differently, and lot 11 cannot draw it without choosing | **a product decision, and it is NOT taken. Three readings, and this line does not pick one.** ⓐ `MOD` shows which CV is routed to this channel **whatever** its destination — the field keeps its place and loses the meaning it has in the original. ⓑ FlexSeq **adds** `SUBDIV` and `SKIP_CHANCE` to the destinations. The format allows it: the code goes in bytes 7 and 8 of the channel record, `NONE` stays 0, so appending 5 and 6 reads any earlier image without migration — but it is a new mechanism, and `docs/original-modes.md` shows it is what the original really does. ⓒ line 3 stays **empty** in CLOCK and RANDOM until the question is settled, which is what lot 11 does by default if nothing is decided. ⚠️ **Whichever is chosen, PRD §12.1 or §10.2 must change**, because today they contradict each other |
