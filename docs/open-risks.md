...
| 89 | **The TypeScript view still draws the OLD FlexSeq layout, and no test asserts an absolute text width.** `sim/src/sim/MainScreenDisplay.ts` holds a headline box of 10 rows at the top of the screen, while the firmware now draws the three lines of the original with a large parameter on the left. The two languages therefore agree on the FONTS — both atlases are decoded from `src/hal/OriginalFonts.cpp`, and eight lines of the vector file of ADR 0012 hold their values in both — and they disagree on the LAYOUT. ⚠️ **Removing the one-pixel tracking left 595 tests green**, which means no test measured a width at all: the removal is right by construction, not guarded | **medium, and it is a gap of the preview, not of the firmware.** The panel proof of `run-screen-dump.sh` covers the firmware. What is missing is the ability of the TypeScript side to contradict it | **step 7 of lot 11**, decided by the owner on 2026-09-04: the mockups move into `sim/` and read the shared geometry file. The hole closes there, because a view built on the vector file can be compared to the panel pixel by pixel |
