...
Fifty-five lines, and each one states **what it is waiting for and from whom**. A line
that waits for nothing from nobody no longer belongs here: it is in the table
below.

| # | Subject | Severity | What is left, and from whom |
| 110 | **The action field offers `SAVE` and a short press on it does nothing.** A decision of the owner of 2026-09-14: `SAVE` is OUT of lot 16E, the budget having no room for it. PRD §5.0 amendment 1bis says a rotation moves between `LOAD` and `SAVE` and that a short press **runs** the action, so the specification and the code diverge on `SAVE` alone. ⚠️ **The value is still OFFERED**, because it appears only when the copy has changed and that rule is PRD §12.9 point 5, which nothing revokes | **low, and it is an absence and not a defect.** Nothing is destroyed and nothing is written: a short press on `SAVE` closes the field. The risk is a reader of the PRD who believes the write exists, and a user who presses and sees nothing | **a lot, and its price is known.** `saveTemplate()` has no production caller, so the linker drops it: wiring it costs the order of `5d`, **200 to 250 bytes**, measured on the same shape three times. The change flag keeps its SECOND role meanwhile, guarding a changed copy at load time, so nothing of sub-step 5b is wasted. ⚠️ **Two routes and neither is chosen**: bring `SAVE` back in a later lot, or hide the value until it runs |
