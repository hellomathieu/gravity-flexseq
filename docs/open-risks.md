...
**Last review: 2026-09-05.** Forty-eight open lines, forty-nine closed or accepted.
...
| 86 | **The reserve of Flash beyond the assumed 30720 bytes stays unmeasured, and the method that would GUARANTEE it is excluded.** The build assumes 2048 bytes are reserved for the bootloader while its measured content is 512. Reading the `BOOTSZ` fuse would say whether **1536 bytes** are free, and it needs a programmer: the bootloader answers `0x0` for fuses. ⚠️ **Flashing through a programmer without a bootloader at all — which would guarantee 2048 bytes — was EXCLUDED by the owner on 2026-09-03**, because it costs the update over USB and the restore path of the original firmware | **medium, and it is now the last lever that could close the worst case.** After the savings of 2026-09-03 the margin is 3429 bytes against an estimate of 2240 to 4580, so the worst case is short by about 1151. The 1536 of this line would close it | **buy a programmer and read the fuse.** The module carries a socketed Arduino Nano, verified in the schematic, so it can be programmed on the bench. ⚠️ **The purchase guarantees KNOWING, not the bytes**: if the fuse reserves 2048 the answer is that there is nothing to take |
