...
**Last review: 2026-08-30.** Thirty-six open lines, thirty-four closed or accepted.
...
|---|---|---|---|
| 65 | **The trigger probe takes the played length from its caller, and does not derive it from the image it validates.** `tools/run-trigger-probe.sh` sends `LENGTH` to `tools/eeprom-image.cpp`, which applies it through `setBaseLength()`, and it sends the same value to `trigger_probe.c`, which uses it for the wrap gap `length - last + first`. The two sides therefore agree because the caller makes them agree. An invocation that gives the SAME wrong length to both would validate an assumption that is self-consistent and false. ⚠️ **This does not weaken the proof of lot SF3.** The counter-proof `EXPECTED_LENGTH=16` against an image of 36 reddens the SEQ course at 17/20, so the parameter does act on the criterion. Written 2026-08-30 at the end of lot SF3 | **low. A tooling debt, not a defect of the firmware and not a defect of the proof.** The method rule of this document already states the general form: a proof tool should not re-enter the assumption it is meant to check | **a deferred improvement, and it belongs to no lot in progress.** Derive the played length from the generated EEPROM image, the way `run-stack-probe.sh` derives the active format from `firmware.elf` through `tools/active-format.sh`. Lot SF3 refused it as a deliberate non-choice: it was larger than the minimal adaptation the lot had authorised |
| 65 | **The trigger probe receives the expected played length from the caller, instead of deriving it from the EEPROM image it validates.** Lot SF3c.5b made `PATTERN_LENGTH` a runtime parameter of `tools/simavr-ssd1306/trigger_probe.c`, and `tools/run-trigger-probe.sh` sends the same `LENGTH` to the image generator and to the probe. The wrap gap of the expected sequence uses that value, so a caller that gives the SAME wrong length to both sides makes the probe confirm a self-consistent but wrong assumption. ⚠️ **This does not weaken the proof of lot SF3.** The counter-proof `EXPECTED_LENGTH=16` against an image of 36 turns the SEQ course red at 17/20 and exits 1, so the parameter is proved to act. What is missing is the independence between what the harness expects and what the image carries | **a tooling debt, and it does not block lot SF3.** The method rule of this document already states the class: a tool must not assume what it measures | **a deferred improvement, and the owner decides when.** Derive the expected played length from the generated EEPROM image, the way `tools/run-stack-probe.sh` derives the active format from `firmware.elf` through `tools/active-format.sh`. Lot SF3c.5a refused it deliberately, to keep the lot inside its perimeter |
