...
| 44 | **A SHIFT burst of more than six detents is not reliable on the LENGTH field, and the mechanism is NOT identified** | **measured 2026-08-25 (D-h, D-i), scope limited, one contradiction open** | on the LENGTH field of a channel tab, a single burst applies **exactly** its detents up to **6**, and becomes erratic from **7**: measured 3, 5, 7, 5, 1 and 4 applied for 7 to 12 requested. The threshold is bracketed to one detent. The loss is **not** the cursor displacement -- the selection frame stays on LENGTH in all fourteen courses -- and it is **not** a timing effect: 6 detents slowed to a 654 ms hold stay exact, 8 detents accelerated to a 222 ms hold stay wrong with the **same** deficit as at 462 ms. The I2C traffic shows the events are emitted, and it is highest on the course that loses nothing. **Do not read a cause into this.** No buffer, no queue size and no firmware component is named by any measurement. ⚠️ **THE SUBDIV EXEMPTION IS REFUTED, measured 2026-09-04.** This line said that a burst of 7 and a burst of 8 apply all their detents on the SUBDIV field. That claim rested on the DESCENDING leg of the `rig` phase and of recipe R11, and **that leg is blind** -- see line 87. The ASCENDING leg of R11 applies **7 detents of 8**: it asks for the rate to return to `/1` (96 ticks per step) and reads 48, so the played step is twice as long as the gesture asked for. The criterion `R11 : cadence restauree` is therefore **RED**, and the date it turned red is **unknown**: this probe belongs to no routine pass. ⚠️ **THE FIRMWARE IS NOT THE CAUSE, and three native replays say so.** The domain alone walks 8 index steps down and 8 up. The real quadrature decoder delivers exactly one callback of amplitude 1 per detent at the timing of the harness, across a direction reversal as well, at four polling rates from 1 to 12 ms. And the whole input chain -- encoder, `EncoderFilter`, `InputAdapter`, `UiController`, `SequencerEngine` -- with SHIFT held and at the exact millisecond schedule of the harness, returns to the starting index every time, and suppresses zero detents. The 9-detent case agrees: the descent clamps and the ascent applies all 9, which is correct. What the simulated run adds, and the replay does not, is the real interrupt path with encoder edges **1 ms apart** while the loop renders and three other interrupt sources compete. That is a hand no user has. What is safe today is a burst of **6 detents or fewer**; what is proven unreliable is a burst above 6 **on LENGTH**. It blocks the restore leg of R5, which targets LENGTH. ⚠️ **R7 targets SUBDIV, not LENGTH** -- `rotate(avr, 2, 1)` selects field index 2 -- so this line says nothing about it: the upper boundary on SUBDIV is unmeasured, and R7 stays unvalidated for want of a measurement, not through a proven defect. See `WORKPLAN.md` ⚠️ **A MEASUREMENT OF 2026-09-05 PUTS THE FIGURES OF THIS LINE IN DOUBT, and it does not refute them.** The harness settled 50 ms after every detent and **nothing before the first one**, so the first injected detent after a quiet period was lost. Measured on the R11 course: a plain wait before the descent decides the outcome, and the threshold sits between **20 and 50 ms**. A sweep of the burst size from 1 to 8 detents applied **zero** every time, so the loss is **not** a function of the size. This line tested timing by varying the HOLD, never the settle **before** the burst, so its own measurements were taken on a harness that could lose the first detent. **The 6-detent threshold on LENGTH must be re-measured** before it is quoted again. The fix is one line, and it took the probe from 91 green criteria to **103**, from 6 defects to **2**, and from 8 undecidable criteria to **0** |
