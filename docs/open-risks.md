...
**Last review: 2026-09-09.** Forty-seven open lines, fifty-three closed or accepted.
...
| 98 | **The two unexplained failures of the gesture probe DO NOT REPRODUCE, measured on 2026-09-09.** The probe reported FAIL on two criteria of its R11 course, on the tree of step 1 of lot 16E. Two runs were then made in a process environment checked clean of any leftover `simavr`: on **step 1 alone** (`10b8dfa`, in a separate worktree) and on **steps 1 to 3** (`a2bd7dd`). **Both give PASS: 106 green criteria, 0 defect, 0 undecidable, RC 0.** The thirteen criteria of R11 are green in both, including the two that were red — the write of the nibble on the first detent, and the restored rate. ⚠️ **The code was therefore never the cause**, and the earlier red belongs to the run and not to the firmware. ⚠️ **The cause of that red is still NOT established.** What is established is narrower, and it is enough to act on: **the run environment is not controlled, and the probe does not check it.** A leftover run of the simulator from an earlier, interrupted pass is the only candidate anybody named, and it is unproven. The FIRST of the three occurrences stays explained on its own: the course restored a subdivision with a burst of eight detents in one go and lost one; staging the burst returned it to PASS | **low, and it is no longer a blocked gate.** The probe is green first-hand on the current firmware, so the condition of `CLAUDE.md` for a first physical flash is met on this check. What remains is a measurement hygiene question, not a firmware question | **nothing from the owner, unless he wants the cause chased.** Two routes exist and neither is needed for the flash: reproduce the red deliberately, with a leftover run of the simulator alive beside the probe, which would confirm or refute the candidate · or make the probe refuse to start while another run of the simulator is alive, which removes the class instead of explaining one case. ⚠️ **Do not read the two PASS as proof that the probe is insensitive to a render change**: it reacted once for a reason that was found, and twice for a reason that does not come back |
