...
| 98 | **The gesture probe reacts to changes of the render that every other check calls correct, and the cause is not identified.** Three occurrences, all on 2026-09-09, all on the R11 course. ⚠️ **The first one IS explained**: the course restored a subdivision with a burst of **eight** detents in one go, and lost one; the descent of the same course, on the same field, is staged one detent per burst and works. Staging the restore returned the course to PASS. That is the burst size the owner already capped at 7 on the LENGTH field (line 44), and the SUBDIV field carries `NO_EMPIRICAL_LIMIT`. ⚠️ **The two that followed are NOT explained.** After the glyph alignment and the header change, the probe reports two defects on the ratchet edit of R11: nothing is written. What is established: the trace is identical up to the entry into the editor · the step grid did **not move by one pixel**, verified row by row — only rows 2 to 6 differ, the title band · the native ratchet tests are green | **high, because it blocks a gate of the first physical flash.** A green gesture probe is one of the conditions listed in `CLAUDE.md` under UI / hardware safety. The probe is the only check that replays real gestures on the production binary, so nothing replaces it. What is at stake is not its result but **knowing when to believe it** | **a decision of the owner, and no lot carries it.** Three routes, none chosen: keep looking for this cause · treat the sensitivity of the probe as a subject of its own, since it has now reacted three times to changes the native tests and the panel call correct · or declare the probe inconclusive on this course, say so in writing, and flash on the other proofs. ⚠️ **Do not weaken a criterion to make it green**: the probe already separates INVALID from FAIL for exactly this reason, and it reported **zero firmware defects** on the first occurrence |
