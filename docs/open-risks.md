...
| 93 | **The gesture probe does not reach the EDIT screen, and it navigates with a field index used as a distance.** Measured 2026-09-05: the witness reads `creneaux 8 -> 8`, so 18 criteria are undecidable across four courses. ⚠️ **One cause was measured and REFUTED, so nobody retries it**: the third grid row of EDIT does NOT fill the tab band. Read from the panel memory, an EDIT screen inks **3 of the 8 tab slots** — the discs sit on line 54, and nothing reaches line 61, while the tab band is lines 56 to 63. The witness is therefore sound, and the navigation is what fails | **medium, and it is a defect of the tool.** It blocks more criteria than any other single line. The screen model draws the tab bar **without any condition**, so the bar level, the tab level and the `CONFIG PATTERN` page all ink 8 slots; only EDIT inks fewer. A reading of 8 therefore means the probe stayed in the main screen. That agrees with `distincte 1` of the same run: the screen did change, and it stayed inside the main screen | **make the navigation absolute instead of relative.** The probe calls `rotate(SEQ_FIELD_INDEX_EDIT_ENTRY, 1)`, which spends an **index** as a number of detents. The two are equal only when the cursor starts on field 0. One detent too many lands on field 2, which is `CONFIG`, and the press then opens the CONFIG page — a distinct screen that still draws the bar. ⚠️ **That last step is a candidate, NOT established.** What is established is the class: an index is not a distance, and the distance is `target - current`. The lot 11 repair replaced hard-coded detent counts with the published indices; it closed the shift and left this |
...
of line 87: a tool that assumes a state it does not own. ⚠️ **The repair of lot
