...
| 70 | **`ZONE_WIDTH = 33` and `ZONE_COUNT = 31` are normative figures of the PRD, and no mutant holds them.** `§10.4` states both, and the mutation probe targets neither `LengthCv.cpp` nor `LengthCv.ts`: the file appears in **0** of the 232 mutants. The 11 tests of `test_length_cv` assert the two values as literals, so they would go red if a value moved. But no pass has ever put them red, and the rule of this document says a green test proves nothing until it has been red. ⚠️ **The asymmetry is what makes this a debt, not a preference.** `MAX_LENGTH`, another figure of the PRD, is held by three mutants: "the length cap falls back to the historical 24 steps" · "the length cap falls to 20 steps" · "the stored bound falls back to the historical cap". The quantiser is treated differently from the rest of the domain, and nothing decided that | **low, and it is coverage and not correctness.** The two values are correct, and the tests do assert them. What is missing is the proof that the assertion bites | **not resolved by D2, and deliberately so.** D2 identified the gap while it arbitrated the normative level of the hysteresis; it was not the place to redo the mutation strategy. Two mutants would close it, one per language, each moving one of the two constants. Recorded at the closure of D2, 2026-08-31 |

| 71 | **Decision D2.3 puts a PROPERTY in the PRD, and no mutant attacks that property.** The owner chose option C on 2026-08-31: `§10.4` states that the hysteresis must stay well above the measured noise of ±3, and it does **not** state `HYSTERESIS = 8`. The literal stays in `lengthcv::HYSTERESIS`, and the tests hold it. ⚠️ **The mutant this calls for is NOT a check that the literal is 8.** It must lower the hysteresis until the measured noise can cross a zone boundary, and then `test_the_measured_noise_never_crosses_a_boundary` must go red. That test already exists, and it is the reason option C was affordable | **low today, and it grows with the value of the statement.** A requirement the PRD carries and nothing holds is a requirement a future lot can break in silence. The full-range alternative of 71 zones was set aside precisely on that ratio, so the ratio is what must be defended | **not resolved by D2.** A mutant that lowers the hysteresis to about 3 should redden the noise test, and a counter-proof must show it green at the current value. ⚠️ **Do not replace it with an equality check on 8**: that would put back in the tests exactly what D2.3 kept out of the PRD. It would tie the proof to a tunable value instead of to the property. Recorded at the closure of D2, 2026-08-31 |

| 69 | **Three sections of the PRD still give `MAX_LENGTH` as 24 in the present tense, and lot SF3 raised it to 36 on 2026-08-30.** `§10.2` says "`MAX_LENGTH` is **24 until lot SF3**, and then 36" · `§10.3` says "24 today, and 36 at lot SF3" · `§11.1` says "the interface cap `MAX_LENGTH`, which is 24 until lot SF3". The code holds **36** in both languages, `SequencerEngine.h:33` and `SequencerEngine.ts:61`. ⚠️ **Two other occurrences are LEGITIMATE and stay**: `§1` records the correction of 2026-08-31, and `§12.1` tells the state between lot F and lot SF3. Both are dated and marked | **medium, and it became ACTIVE with the rewrite of §10.4 on 2026-08-31.** While §10.4 said nothing about lengths, the sections did not meet. §10.4 now builds its partial-reach table on 36 lengths, and `base 36 → 21 to 36` cannot hold under a cap of 24. The three lines were already listed as stale during the translation pass, and the method left them intact on purpose | **one factual commit, and it belongs to no translation pass.** The three lines carry the same stale fact, so they are corrected together rather than one section at a time. ⚠️ **Do not fold this into a rewrite of a section**: the owner set the perimeter of D2 at §10.4 alone, and a transversal correction inside a section rewrite mixes two pieces of work. Until then, §10.4 is coherent with the implementation and temporarily incoherent with two of its neighbours. Recorded at the closure of D2, 2026-08-31 |
