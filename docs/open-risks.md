...
| 91 | **The stack peak of the production firmware is 222 bytes against a reserve of 256, so 86 % used and 34 bytes of margin to that reserve.** Measured 2026-09-04 after routes 1 and 2 of lot 11: 220 on the nominal course, **222 on a preloaded CLOCK image with the CV reset path**, and 26 on any SEQ image. ⚠️ **The cause of the spread is established and it is the MODE of the image, not the duration** — verified at 8 s and 16 s: a SEQ image does not draw the three-line layout, so it never enters `measureMainScreen` → `getStrWidth` → u8g2's glyph decoder, which is the deep part. CLOCK is the module's default mode, so 222 governs. ⚠️ **What is NOT explained is the SIZE of the spread**, 26 against 222: the SEQ path still calls `getStrWidth` for its headline, so 26 is lower than I can justify. Recorded as a signal rather than smoothed | **medium, and it is a watch item.** The absolute condition holds widely — 222 against 537 bytes of free RAM. What has shrunk is the comfort the reserve was chosen for: it covered the peak 2.1x in August, 1.2x in September, and **1.15x** now. A stack overflow on AVR is silent corruption, never a link error | **a rule for the next lots, and a measurement to redo.** Any lot that adds call depth runs `run-stack-probe.sh` **before** it accepts its footprint — the rule exists and it was broken twice inside lot 11 alone. And the 26-byte reading deserves one hour: either it is correct and the SEQ render is genuinely shallow, or the probe under-reports on that path, which would put every SEQ-image figure in doubt. ⚠️ **The D79 witness of the RESET course needs a SEQ image**, one active step, so the 222-byte run does not check the reset property; that check lives on the SEQ run at 26. Neither run replaces the other |
