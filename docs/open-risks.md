...
| 14 | **Three audited libGravity anomalies are latent and land exactly on the next task.** `Button` can lose a release inside the debounce window; `Encoder` has an unsafe initial state and may report a false first movement; `Clock::SetSource` does not handle the `SOURCE_LAST` sentinel (it surfaces today as a compiler warning) | medium | none of the three is reachable today -- no callback is attached and no clock source is selected (line 11b). They become live the moment the UI and the transport are wired, which is the adapter layer's job. Written here so that wiring does not rediscover them |
| 15 | **`DigitalOutput::Init()` does not force the output OFF**, and the audited test proves it | low | **harmless at cold boot, and for a reason worth stating**: an AVR reset leaves `PORT` at 0, so `pinMode(pin, OUTPUT)` drives the pin LOW, and `gravity` is a global in `.bss` so `on_` starts false. It becomes real on a **soft reset** -- the original firmware has a `reboot()` -- or on a second `Init()`: the pin could stay HIGH while the firmware believes it is off, and nothing would ever clear it. To handle if a reboot is ever added |
