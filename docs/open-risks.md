...
**Last review: 2026-09-05.** Forty-seven open lines, forty-eight closed or accepted.
...
| 92 | **The gesture probe assumed the depth between a screen and the tab bar, and lot 12 added a level.** `alignTab()` only rotates: it requires the caller to be on the bar already, and every course returned there with ONE long press. From the `CONFIG PATTERN` page that reaches the tab, not the bar, so the next `alignTab()` turned the FIELD cursor instead of the tab. Measured: R1 read tab 1 where it aligned to tab 2 | **FIXED on 2026-09-04.** `backToBar()` sends three long presses, which is idempotent — a long press on the bar is a no-op, `handleTabBar` having no `LONG_PRESS` case — so the harness stops assuming a depth it does not control. **The exchange is measured and it is a net gain**: green criteria 69 → 81, criteria left undecidable downstream 30 → 18, defects 4 → 5. The two that appeared, R13 and R2, were **masked** before, not newly broken | **three criteria remain, and the measurement of 2026-09-04 sharpens them.** The `SUBDIV` edits of three courses read an unchanged step. ⚠️ **The account 'the first burst applies and the following ones do not' is REFUTED by the staged descent of R11**: a burst of 7 detents applied **one**, and the single detent that followed applied **one** as well — 96 → 48 → 32 ticks, so index 8 → 7 → 6. The rig walk of the same run applies its whole burst of 7 (96 → 6 ticks), so the two paths differ and the cause is **not** established. `shiftRotate` was read and it emits no detent while SHIFT is released, so that hypothesis is refuted; the swallowed first detent of the encoder (upstream defect 3) is the next candidate and is **not** established. R11's blind descending leg is CLOSED, line 87, and the root of the 18 undecidable criteria is line 93 |
| 93 | **A channel tab in CLOCK or RANDOM mode has no EDIT field and no CONFIG field, so the gesture probe cannot reach the EDIT screen.** `UiController::fieldAt()` serves two maps. `isLegacyModeTab()` is true when the mode is CLOCK or RANDOM, and the three fields are then MODE, OFFSET or SUBDIV, and MOD. Only a channel in SEQ mode carries MODE, EDIT and CONFIG. The factory default is CLOCK, so the courses that use the factory image can never enter EDIT. Measured 2026-09-05: 18 criteria are undecidable across four courses | **HIGH, and it is a defect of the firmware, not of the tool.** ⚠️ **A second inconsistency is established by reading, on a SEQ tab.** The controller gives a channel tab three cursor positions, `CHANNEL_TAB_FIELDS = 3`, and maps them to MODE, EDIT and CONFIG. The screen draws the other layout there — LEN, SUB, SEP and EDIT, keyed to cursors 1 to 4. So cursors 3 and 4 are unreachable, and the two that are reachable carry labels that do not name what they do. ⚠️ **Two earlier explanations are REFUTED, so nobody retries them.** The third grid row of EDIT does not fill the tab band: an EDIT screen inks 3 of the 8 tab slots, read from the panel memory. And the navigation is not the cause: an absolute navigation was built and measured, the cursor reached field 1 — label ink 161 against 51 and 37 — and the press still did not enter EDIT | **the owner decides, and the two parts are independent.** The probe cannot be repaired alone: a course that wants EDIT must first put the channel in SEQ mode, which is a gesture the courses do not perform today. And the SEQ tab layout must agree with the controller before any course can name a field by what the screen shows. ⚠️ **The absolute navigation was written and then REVERTED**: it made the probe worse, 81 green criteria down to 56, because the cursor reader assumed the three-line geometry while a SEQ tab draws the four-field one. Only the `KEEP_WORK` lever of the script survives, which keeps the raw phase logs and is what made the diagnosis possible |
