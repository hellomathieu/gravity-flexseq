...
| 87 | **The descending leg of the R11 course of the gesture probe cannot detect a wrong detent count.** `kSubdivChoices` is clamped at both ends and places `/1` at index 8, and `FIELD_SUBDIV` uses `clampIndex`. A burst of 8 detents down from index 8 lands on index 0. A burst of 12 lands on index 0 as well. The criterion `R11 : cadence x24` therefore passes for **any** burst of 8 detents or more, and it hides a miscount instead of reporting it. Only the ascending leg can detect one, because index 8 sits inside the list. Written 2026-09-04, when the ascending leg was found red | **low, and it is a defect of the tool and not of the firmware.** The criterion is not wrong about the value it reads. It is unable to fail. The method rule of this document already states the class: a tool must not assume what it measures | **a small change to the harness, and the owner decides when.** Two items, and they are independent. Give the descending leg a target inside the list, so that an extra or a missing detent moves the result. And give the harness a lever for `EDGE_SPACING_MS`, which is a hard-coded 1 ms today: without it, the interrupt hypothesis of line 44 can be neither proved nor refuted. ⚠️ **`DI_SETTLE_MS` does NOT serve for this**: raised to 70 ms it makes the 12-detent burst of another phase exceed the 750 ms ceiling of the SHIFT hold, and the harness refuses before injection, exit 4. Measured 2026-09-04 |
