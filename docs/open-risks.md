...
| 90 | **The channel tab of the original takes about 13 ms in one loop pass, against the budget of 12 ms of ADR 0001.** Measured 2026-09-04 by `run-blocking-probe.sh` on `env:mainscreen`, 8 s: **p90 13.02 ms, median 9.60, worst pass 13.05**, band transfer 3.38 ms, frame 57.5 ms. The same probe, same duration, on the EDIT screen of `env:wokwi` reads **6.28 ms** with an identical band transfer of 3.36 — so the whole gap is DRAWING, not the bus. **Two causes are attributed by counter-build, and one hypothesis was refuted**: drawing the 23-pixel `stkL` value costs **2.11 ms** (removing the draw while keeping its width measurement gives 10.91 ms, under budget), and the five `getStrWidth` calls cost **0.91 ms** together (neutralising all five gives 12.11 ms). ⚠️ **The `getStrWidth` hypothesis was the first one and it was WRONG**: the renderer calls it up to six times per band where the EDIT screen calls it zero, which looked like the answer and accounts for less than a seventh of the gap | **medium, and its frequency is the mitigating fact.** Outside `EDIT`, production redraws only when `ui.revision()` changes — established on `src/main.cpp:270`, where `step` stays at -1 and `revision` is the only trigger. So the over-budget pass happens on a **gesture**, not on every frame. ADR 0001's budget is on the pass and not on the frequency, so the miss is real; what it costs is a longer blind window on a gesture, not a permanently slow loop | **a decision of the owner, and three routes with measured prices.** (a) Carry the widths in `MainScreenModel`, as `PatternScreenModel::titleWidth` already does for the same reason: returns **0.91 ms** for a few bytes of RAM, and takes the pass to 12.11 — still over. (b) Raise `PASS_BUDGET_MS` for a screen that redraws on a gesture only: costs nothing and changes the contract of the probe. (c) Accept it and record it. ⚠️ **Giving up the large font is NOT on this list**: the owner chose the typography of the original on 2026-09-04, and its 2.11 ms is what drawing a 23-pixel glyph costs. ⚠️ **Two limits of the measurement, named.** `env:mainscreen` never samples the CV, so the ADC correction could not be computed — line 59 — and 13.02 is the uncorrected simulated figure; on hardware the ADC adds about 5.6 % of CPU, which would not reduce it. And that environment renders bands in its own loop without skipping any, so its FRAME time is not production's; its PASS time is, the work per band being the same `drawMainScreen` call |
