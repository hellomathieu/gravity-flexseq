...
**Last review: 2026-08-31.** Forty-three open lines, thirty-three closed or accepted.
...
| 79 | **After a global reset, the step where the playhead lands never emits its entry onset.** `advance()` emits the onset of a step at the boundary crossing that ENTERS it (`SequencerEngine.cpp:169-177`). A reset places the playhead without a crossing. So after PLAY or MIDI Start, step 0 occupies its duration and its trigger stays silent; the first emitted onset belongs to step 1, one step after the start. The transient lasts one step: from the first loop of the pattern, step 0 is entered by a crossing and sounds. Found on 2026-09-01 during the design of the CV RESET (lot E3.7-F3, step F3.4). ⚠️ **Seen on the pins since 2026-09-02**: the reset stack course presses PLAY on an image whose only active step is 0, and OUT1 stays silent until the injected edge — the first grid front falls one full cycle later. The trigger probe alone could not see it: it checks CYCLIC ROTATIONS of gap sequences, which a missing first onset does not change | **a product question, not a defect of the CV RESET lot.** The CV RESET satisfies §10.3 with its own armed mechanism, per channel, without a change to the global path | **a decision of the owner, out of the perimeter of E3.7-F.** Aligning the global reset on the armed mechanism would change a measured behaviour — the drift probe counts 222 fronts — and would touch §8 of the PRD. This line closes when the owner decides whether PLAY and MIDI Start must sound step 0, and the answer enters the PRD at that moment |
| 80 | **A CV reset armament survives a change of mode, and its consumption follows the onset predicate.** A channel armed in SEQ can change to RANDOM before the next `advance()`. The armament is consumed there, the onset is emitted, and the RANDOM draw decides the trigger. The F3 design does not name this case. The implementation follows the F3.4 pseudo-code to the letter and adds no mode guard: an unspecified guard would be a potential equivalent mutant. Named during F4.2, 2026-09-02 | **low, and it is a semantic question, not a defect.** The window lasts one loop pass, about 6.6 ms at p90, and it needs a gesture and an edge inside the same pass | **a decision of the owner.** Three options: keep the literal behaviour, clear the armament on a change of mode, or clear it only on the exit from the matrix. The chosen answer enters the F3 contract, with its test and its mutant, when it is made |
