...
| 23 | **Restoring the original firmware has never been run, and the original is already off the module.** The backup exists and is verified, but restoring it is a claim, not a fact. PRD §17 and §19 make that ability a project constraint | **dominant** | **half closed 2026-08-22**: `tools/run-original-restore.sh` exists, and its software half is verified -- trim, integrity, byte-for-byte re-read, three paths exercised including two red ones. What remains is the run itself, which needs the module and the owner. Why the trim is mandatory, and it is not merely a verification concern: optiboot occupies 512 bytes at `0x7E00`, it has no self-write guard, and that region is NRWW on the ATmega328P, so the CPU halts while its own page is being erased -- the attempt can leave the module with no bootloader at all, recoverable only with an ISP programmer. Reasoned from the AVR memory model, deliberately not measured. What **is** measured on the backup: `0x6C00`-`0x7DFF` is entirely `0xFF`, 4608 bytes, so trimming to `0x6BFF` loses nothing. Closing criterion: the original boots, and **the owner's eight patterns and his 120 BPM are still there**, which proves the restore and the untouched EEPROM in one go |
