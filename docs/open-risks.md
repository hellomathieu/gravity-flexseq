...
**Last review: 2026-09-14.** Fifty-three open lines, fifty-five closed or accepted.
...
| 103 | **A power cut during a template edit leaves channel 1 in `SEQ`.** The template editor takes channel 1 into `SEQ` while it is open, and gives the mode back when it closes — PRD §5.0, amendment of 2026-09-13, point 4. A channel in `CLOCK` or `RANDOM` reads no pattern, so the audition needs it. The periodic scan of the persistence writes the mode of every channel, so a cut between the two moments persists the forced mode | **low: no data is lost and nothing breaks.** The user finds channel 1 in `SEQ` instead of the mode it had, and one gesture puts it back. Nothing else changes | **nothing, and that is a decision of the owner of 2026-09-13.** The alternative was named and set aside: to leave the mode alone and to say on the screen that the audition is silent. The owner chose the audition that always works. ⚠️ **Do not reopen this line as a defect**: it is an accepted consequence, and it is written in the PRD |
| 104 | **The length byte of the modulation buffer is written by the loader and read by nobody.** Found on 2026-09-13 by reading the code. `loadTemplateIntoModulationBuffer()` fills `state.length[channel]` (`include/flexseq/Persistence.h:470`), and a search over `src/` and `include/` finds no reader. A channel whose pattern a CV modulates therefore changes the **content** it plays and keeps **its own** length | **low, and it may be exactly what the specification wants.** PRD §5.2 gives the length to the channel, and the CV pattern path is forbidden to touch `baseLength`. The risk is that six bytes of RAM look like state and carry none | **a reading of the specification, and no lot carries it.** Two outcomes, and each is cheap. Either the behaviour is intended, and the field goes, which returns 6 bytes of RAM · or the played length must follow the modulated template, and the engine gains a reader. ⚠️ **Nothing is decided here**, and the template editor of lot 16E step 4 uses that byte for its own purpose, which neither settles nor blocks the question |
