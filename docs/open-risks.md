...
**Last review: 2026-09-02.** Forty-six open lines, thirty-four closed or accepted.
...
| 81 | **Nothing establishes that a template record of a GENERATED EEPROM image can be loaded into the modulation buffer.** `loadTemplateIntoModulationBuffer()` refuses a record whose length byte falls outside `[1, 36]`, so the chain `generated record -> length field -> isValidTemplateLength() -> loadable` decides whether a preloaded image can feed the buffer at all. ⚠️ **Two observations conflict, and the status is UNKNOWN.** `run-eeprom-image-check.sh` asserts "16 on the sixteen records", which is a green tool; a manual read of the generated bytes gave 0. The manual read rests on an offset that was never established, and three successive manual readings of that image gave three different answers, so it carries no weight. The green tool carries weight for its own criterion, and it says neither where those bytes sit nor that the generator writes them into the expected field | **medium, and it BLOCKS a proof rather than a feature.** No production path is suspected: the firmware seeds the templates itself, and the modulation service has run on the pins. What is blocked is the trigger-probe course that would prove the B2 candidate of lot STEP | **establish it with the tooling of the project**, `addressAt()` and the coverage check of the generator, and never with a hand-read offset. Lot STEP, step 7.6bis.1 and 7.6bis.2. ⚠️ Until then, write neither "the lengths are zero" nor "the lengths are valid" |
