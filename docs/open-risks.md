...
**Last review: 2026-09-03.** Forty-three open lines, forty-six closed or accepted.
...
| 84 | **Lot K would LOWER the maximum tempo, and a stored value above the new bound has no defined behaviour.** The interface accepts `[30, 300]` today — `UiController::MIN_TEMPO` is 30 and `MAX_TEMPO` is 300. The development plan describes lot K as "tempo 20 to 200", so it lowers the maximum by 100 as well as the minimum by 10. A tempo between 201 and 300 is therefore storable now and out of range afterwards, and nothing says what the load does with it. Found on 2026-09-03 by condition 8 of lot S2.1-b, when the generator refused a tempo of 20 with `tempo refused: 20` | **medium, because it is a persistence question and not only a bound.** The same class as the two entry points of the length, which ADR 0009 settled by keeping a storage bound apart from the interface bound. Here no such separation exists for the tempo | **a product decision at the start of lot K**, and it has three parts: whether the target is really 20 to 200, what the load does with a stored 201 to 300, and whether the tempo needs a storage bound of its own as the length has. ⚠️ **The baseline of condition 8 was therefore measured at 30, 120, 200 and 300**, the real bounds, and not at the bounds lot K announces |
