...
**Last review: 2026-09-07.** Forty-six open lines, fifty-one closed or accepted.
The two counts are read from the tables themselves. The header carried forty-eight
and forty-nine until 2026-09-07, and the body of the first table already said
forty-six.
...
| 61 | **`main` is 6370 bytes and `PagedScreen::renderFrom` is 3048, and these are MEASURED CONSUMERS, not demonstrated savings.** Both figures come from `avr-nm` on the ELF of 2026-09-04, after step 5b-ii of lot 11. Together they hold **9418 bytes, so 34 % of the Flash in use**. ⚠️ **Their size says nothing about what a change could return.** `main` holds `setup()`, the loop, `bootstrap()` and everything LTO inlined into it; `renderFrom` is the spread renderer of ADR 0001 with both screen layouts inlined into it. The figures of 5976 and 2490 that this line carried until 2026-09-04 are superseded | **an opportunity, and lot S3 has now examined it.** ⚠️ **Its answer is that there is nothing more to take without giving something up**, and six counter-builds support it. The budgets pass: Flash **91.4 %**, and **2013 bytes** of margin before the 98 % guard, re-measured 2026-09-07. The estimate of the remaining work reads **1990 to 4030 bytes**, so the margin still sits at the low end. ⚠️ The figures of 90.9 % and 2195 bytes belonged to 2026-09-04 | ✅ **THE OWNER OPENED A DEDICATED LOT ON 2026-09-04, lot S3 of `WORKPLAN.md`.** It delivers an **analysis** and no code: what is inlined into each of the two, block by block, with a measured estimate for every candidate. ⚠️ **The analysis must be allowed to conclude that there is nothing to take** — otherwise it presses itself into finding savings that do not exist, and this line has said since 2026-08-30 that **no gain must be assumed**. QB3 and lot S2 excluded both twice, deliberately, so that the gains of those lots stayed attributable; that reason no longer holds, because the margin now does |
