...
| 96 | **The SELFTEST of the gesture probe has not run since 2026-08-29, and its expectations no longer match what the probe reports.** It replays thirty-two verdict cases and three source mutations, and each one expects a verdict, an exit code and a cause. The probe has changed a great deal since: lot 11 moved sixteen field indices, lot 12 added a level of navigation, and 2026-09-05 added the `MOD` course and two criteria. Deliberately not re-run on 2026-09-05, and said so rather than passed over in silence | **medium.** It costs about 68 minutes, so it belongs to no routine pass, and that is exactly how it went stale for two days once before — line 57, closed. A guard nobody runs stops being a guard | **run it, and repair what it names.** Its verdict counts are almost certainly stale, since the nominal pass went from 103 criteria to 106 over the same period. ⚠️ **Do not read a failure of it as a defect of the firmware** before checking which of its cases moved: that confusion is what line 57 recorded |
