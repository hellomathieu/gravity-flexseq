...
**The count is three again since 2026-09-07**, and for a different reason: the amendment above, and not the state of 2026-08-22. The released original has **three** modes, `byte mode; //0 - CLK, 1 - RND, 2 - SEQ` (`Gravity.ino:63`). `GATE` is a mode of the `1.2-dev` branch, so it lives in this catalogue and it carries no requirement of its own. To bring it back is a product decision, and it goes to `PRD.md`, never to this file.
