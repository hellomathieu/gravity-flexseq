...
- **`SKIP CHANCE`**: `random(10)+1 > randAmount`. ⚠ **Two distinct bounds, found on 2026-08-23**: the original interface clamps the **stored** value to **9** (`Interactions.ino:157-161`), so 0 % to 90 %. The **generation** clamps `random + randMod` to **10** (`Gravity.ino:479-483`), so only the CV modulation reaches 100 %. FlexSeq let the user set 10 directly until 2026-09-06. **The owner settled on the cap of 9** (§16), and the decision is **applied since 2026-09-06**: `MAX_SKIP_CHANCE_SETTING` is 9 and bounds the interface, `MAX_SKIP_CHANCE` stays 10 and bounds the engine, so the effective value keeps 10 through the CV;
...
**6. Tab bar: nine tabs and one indicator — IMPLEMENTED on 2026-09-09, lot 16E steps 1 to 3.** ⚠️ **THE BAR HOLDS EIGHT TABS SINCE 2026-09-14**, the third amendment of the day having removed the `PATTERNS` tab. Eight slots make 96 px, and the indicator keeps its place at `x = 121`. Read the rest of this point as the state of 2026-09-09. `clock · 1 2 3 4 5 6 · PATTERNS · CONF`, and then the **transport indicator flush right**. The slots are **12 px**, the value of the original firmware (`xWidth = 12`, `UI.ino:238`). Nine slots make 108 px, and the indicator sits at `x = 121`, so the geometry of the original holds as it is. ⚠️ **This line said 12.9 px until 2026-09-06**, a value derived by a division and never read from the original. The owner settled on 12 px on 2026-09-06.
...
- ⚠️ **THE GLYPH OF `CONF` IS NOT A COGWHEEL, and this line said a 7×7 cogwheel until 2026-09-09.** §12.1 decision 4 superseded that shape on 2026-09-04, and this line kept the old one. The glyph is **two sliders**. The owner chose the size on 2026-09-09: **7 px wide and 5 px high**. Two bars run the full width of the glyph. Each bar carries one knob, above the first bar and below the second. The filled square of before reads as a stop indicator, which the owner verified on the module. To leave it two slots away from a real indicator would bring the ambiguity back, side by side;
- the glyph of **PATTERNS is a small grid of steps**, two rows of three dots in 7×5 px. ⚠️ **THIS GLYPH IS REMOVED since 2026-09-14**, with the tab it named. The bullet stays because the next one derives the size of the `CONF` glyph from it. ⚠️ **A DOT IS ONE PIXEL, and it was two pixels high until 2026-09-09.** Six dots of 1×2 make three broken vertical bars. They do not read as a grid. The owner read that on the render of the pixel mirror. The two rows of dots sit on the rows 58 and 62. ⚠️ **This line said 58 and 61 for one commit**, before the render was read: on 58 and 61 the glyph floats 1 px above the bottom row. On 58 and 62 it shares the bottom row with the digits, and it fills the 7×5 box this line declares;
- **every glyph of the bar lives in the band of the digits, the rows 58 to 62.** No glyph of the bar is 7 rows high. ⚠️ **An alignment on the bottom row is NOT needed, and a measurement says why.** The firmware embeds the `velvetscreen` font of the original, 436 bytes, identical byte for byte. The clock of the original is the character `w` of that font, and it is 5 px by 5 px. Play is `r`, 3 px by 5 px. Stop is `t`, 5 px by 5 px. The original draws the three of them on one baseline (`UI.ino:238`, `:263` and `:266`).
- ✅ **What the three steps cost, measured on `tools/memory-baseline`**: the nine slots and the band of the glyphs RETURN **88 bytes** of Flash, the three glyphs RETURN **32** more, and the indicator with its redraw trigger costs **72**. Flash goes from 28092 to **28044**, and RAM from 1501 to **1503** — one byte for the run state in the screen model, one for the state of the trigger. The stack peak does not move: **201 bytes**, with 6 of 6 watched vectors entered.
...
**AMENDMENT OF 2026-09-07 — two features are set aside, one is deferred, and the room of the three is reserved.**

**A decision of the owner.** It supersedes point 4 above. It sets nothing aside for good, and the paragraph on the reserved room says at what price each one comes back.

**What is SET ASIDE, and leaves the specification:** **SWING** · the **GATE** mode. To bring either one back is a **new product decision**, and it comes to this section.

**What is DEFERRED, and stays a requirement:** the **mute** per channel. ⚠️ **The difference is not one of wording.** The mute stays in the specification and point 5 above stays exact: `SHIFT` + `PLAY` mutes the channel. Only its delivery waits, and it waits for a lot and not for a decision. Its cost stays owed in the budget: **to defer saves nothing, it only moves the moment of the spend.**

**Why: the Flash budget.** Measured on 2026-09-07: **2013 bytes** stay before the 98 % guard, against **1990 to 4030 bytes** estimated for the work that remains, the margin at the bottom of that range being 23 bytes. **After this amendment the work that remains is 1740 to 3480 bytes**, the mute included, so the margin at the bottom becomes **273 bytes** and the worst case still lacks 1467. Every saving the firmware could give has been taken, and lot S3 measured that there is nothing more to take without giving something up.

**Why these three, and not others.** They are the only three that do not come from the published original. `main` @ `40d4aac` is the behavioural reference of this project, and it holds none of them: they come from `1.2-dev`, a branch that is older, that was never merged, and that the module most likely never ran. **To set SWING and GATE aside moves FlexSeq towards its reference, and not away from it.** ⚠️ **That argument does not carry the mute, and that is why the mute is only deferred.** The mute changes no musical behaviour and it costs one bit: it is a convenience of use, not a divergence from the reference. The owner kept it as a requirement on 2026-09-07.

**The estimated saving is 250 to 550 bytes** — SWING 150 to 300, GATE 100 to 250. ⚠️ **The mute is not in that sum**, because a deferred requirement is still owed: its 20 to 80 bytes stay inside the work that remains. ⚠️ **It is an estimate and not a measurement**, and the three lots delivered so far each cost more than their own estimate, from 2.7 to 12 times. `WORKPLAN.md`, section `RM.16`, holds the table and that measurement.
...
- **`SHIFT` + `PLAY` stays assigned to the mute**, and it does not become free. Point 5 above is untouched. RECORDING stays on `SHIFT` plus a short press, and the reason holds on its own: the handler of a long press runs **on the release**, so a `PLAY` held for 750 ms would arm RECORDING instead of starting the transport;
...
- **The mute takes ONE bit of byte 4**, and that bit is **reserved for it**. Its delivery costs no growth of the record, no change of version, and no return to the defaults. It costs what it would cost today. Being deferred and not set aside, it needs **no decision** to be built: a lot suffices;
...
**1bis. `LOAD` and `SAVE` are two values of ONE field, and that field is the big value of a channel tab.** ⚠️ **THIS REPLACES A FIRST WORDING OF THE SAME DAY**, which made `SHIFT` plus a rotation load on every detent. That wording contradicted point 2 above — "a rotation alone never loads", and "the deliberate action is what keeps that destruction out of reach of one detent" — and it is withdrawn.

The gesture that names a template does not change: `SHIFT` plus a rotation moves the number from `A1` to `B8`, and it loads nothing. What changes is that the **cursor can now stop on the big value**, which carries that number and the label `PATTERN`. A short press **opens** the field, a rotation moves between `LOAD` and `SAVE`, and a short press **runs** the action. No gesture is invented: to open a field, to turn inside it and to validate is what every other field already does.

**The field exists on a channel in `SEQ` only.** The other two modes read no pattern, and `EDIT` is already reachable from `SEQ` alone, by the decision of 2026-09-05.

**The big value is the FIRST position of the cursor**, by a decision of the owner of 2026-09-14: it is the first value to choose before a pattern is edited. The three published indices — `MODE`, `EDIT` and `CONFIG` — move from 0-1-2 to 1-2-3. The gesture probe reads them by name from the domain, so it follows on its own.

⚠️ **A change of mode puts the cursor back on `MODE`.** A channel in `CLOCK` carries `MODE` at position 0, and that position becomes the big value when the channel turns to `SEQ`: the cursor would stay where it is while the field under it changed, and a short press would then open `LOAD` instead of `MODE`. This is the only place in the firmware where a gesture moves what the cursor names, so the controller puts it back.

**A destructive action asks through the FIELD, and not through a window.** A short press on an action that destroys something replaces the label by `SURE`. A second short press runs it. A rotation or a long press cancels. ⚠️ **No screen is drawn and no level of navigation is added**: the field already opens, already turns and already validates, so the confirmation costs a label, a bit and two branches instead of a modal. §12.9 point 3 asks for a confirmation on the screen, and this is one. ⚠️ **The inverted label was weighed and REFUSED**: inversion already means "this field is open", and a second meaning would make the screen ambiguous — the very defect the owner read on the module on 2026-09-13. ⚠️ **THE WORD LOST ITS QUESTION MARK ON 2026-09-14, and the font is why.** This section said `SURE?` until then. `velvetscreen`, the label font of the original, carries NO question mark: the panel of `env:mainscreen` drew `SURE` followed by a blank, and the highlight box was one glyph too wide. The owner chose to keep the word and to leave the font alone, rather than to add a glyph the original never had.

**`SAVE` appears only when the copy has changed**, which is §12.9 point 5 unchanged. On a clean copy there is nothing to publish, so the field carries `LOAD` alone.

**A load that would destroy an edited copy asks first.** Every channel carries a change flag, one bit per channel. **The length of a channel counts as a change**, by a decision of the owner of 2026-09-14: the template record stores a length on its 24th byte, so a channel whose length was edited no longer matches the template it loaded. ⚠️ **The flag rises on the INTERFACE path only.** `setBaseLength()` has seven callers and six of them are not a gesture — the defaults at boot, the entry and the exit of the template editor, and the restoration from the image. A flag raised inside the engine would rise on every one of them. A clean copy loads with no question. A changed copy raises the confirmation window of §12.9 point 3. A successful load clears the flag, the copy being the template again. ⚠️ **The change flag of §12.9 point 5 gains a SECOND role.** It served the `SAVE` button, and it now also guards the load. The amendment above says it serves that button and not the editor, and that stays exact: the template editor carries its own flag, for the template it writes.

⚠️ **THE DESTINATION SELECTOR OF §12.9 DISAPPEARS, and that is a saving.** The destination of a `SAVE` **is** the big value, chosen by the gesture that already chooses it. There is no second screen to draw. ⚠️ **And point 7 of §12.9 becomes true by construction**: the choice of the destination never destroys the edited copy, because turning only changes a name.
...
⚠️ **The rule that put the cursor back on `MODE` DISAPPEARS, and the mutation probe is what established it.** The rule was written for position 0, which the big value took. It is now a no-op, and the proof is an enumeration: the mode changes through `adjustFieldValue(FIELD_MODE, …)`, which is reached only through the field under the cursor. `mainField()` never names `MODE`. So the cursor already sits on `MODE`, which is position 0 in the three modes, and the assignment writes the value it reads. ⚠️ **A first wording of this amendment said the rule stays**, on the ground that position 1 carries `OFFSET` in `CLOCK` and `EDIT` in `SEQ`. That case cannot arise: to change the mode, the cursor must already be on `MODE`. Two mutations of that line went undetected, and they are equivalent mutants.
...
**A load that would destroy an edited copy asks first, and the answer is a choice.** The label of the big value becomes `SURE: NO`, and the big value keeps showing the template that would be loaded, so the question names what it is about. A rotation switches the word to `SURE: YES`, and a short press runs the word that is shown. **`NO` comes first**, by a decision of the owner: one short press too many must destroy nothing. A short press on `SURE: YES` loads. A short press on `SURE: NO` closes the field and restores the name of the template that the channel plays.

⚠️ **ONE WORD AT A TIME, AND THAT IS A MEASUREMENT.** A first wording of this amendment showed both words, `SURE: YES / NO`, with the selected one inverted. It needed a drawing in four pieces, and it measured **372 bytes of Flash against 64**. The image reached 99.8 %, with 76 bytes before the linker refuses, which is not shippable. The owner chose the single word on 2026-09-14, with the two figures in view. **The gesture does not change**: turn to change the answer, press to run it, and `NO` comes first.
...
**A private bank of 16 per channel is ABANDONED.** It diverged from the reference, and it cost about **560 B of RAM**. ADR 0006 cites this section three times, and it takes the provenance of that alternative from here.
...
> **Engine:** the `SequencerEngine` plays, for each channel, its own instance or its modulation buffer (§5.0). At every step boundary it caches the duration (`stepTicks`) and the trigger count of the step, read at `readStep` (§10.3). The read-coordinate calculation therefore does not run on every tick. A previous attempt taught that lesson: a 16-bit division per tick had shifted the simavr signal from 40 to 43 ms. `onsetCount(ch)` exposes the number of triggers of the last `advance()`. `masterPhase` stays unchanged.
...
**Engine API, tested:** `start()`, `stop()`, `reset()`, `advance(ticks=1)`, `refreshTiming()`. Per channel: `selectedPattern`, `effectiveLength`, `subdiv` and `ticksPerStep`, `barLength`, `effectiveStep` (the playhead), `currentReadStep` (the read coordinate, §10.2), `hasStepped`, **`onsetCount`** (the triggers of the last `advance()`, ratchets included), `currentStepTicks`, `currentStepTriggers`.
---
## 8. Transport
...
**A global reset ARMS the entry onset of step 0 — decision D79, 2026-09-02.** `start()` and `reset()` put every channel on its local step 0 and arm it. The first tick after the reset emits the entry onset of that local step. Its content and its ratchet are those of `readStep` (§10.2). That step is step 0 when STEP is inactive or its zone is zero. Otherwise it is the step selected by the STEP offset. The trigger then follows the predicate of the mode at that moment (§4.2, §9). `resume()` arms nothing. This is the behaviour of the original: `resetClocks()` puts the cursor to 0, and the first `clock()` serves the current step before the increment. Before this decision the first onset belonged to step 1, and the armed mechanism of §10.3 carries the alignment.
...
> **Decision validated.** A channel emits a **trigger** when it crosses an onset and the step it reads is **active**: `triggered(ch) = onset(ch) ∧ P(ch).step(readStep(ch))`. `P(ch)` is the pattern played by the channel. It is the channel instance, or the modulation buffer when PATTERN CV modulation is active (§5.0, §10.2). `readStep(ch)` is the read coordinate of §10.2. It equals `localStep` when no CV is routed to STEP. The firmware turns that into a pulse on the output (`DigitalOutput.Trigger()`, 5 ms by default).
...
⚠️ **THE MUTUAL EXCLUSION IS LIFTED — decided by the owner on 2026-08-22.** A channel can route **CV1 AND CV2 at the same time**, each one to its own destination. Two modulations that target the **same** destination **add up and then clamp**, which is a direct generalisation of the "additive offset and then clamp" below. STEP wraps instead (§10.4).
...
⚠️ **The two destinations of the table above that do not exist — `SUBDIVISION` in `CLOCK` and `SKIP CHANCE` in `RANDOM` — are NOT in lot 13, by a decision of the owner on 2026-09-05.** Lot 13 wires the field on the four destinations that already exist and are measured on the pins. The two others need a quantisation rule that §10.4 does not carry: its grid is dimensioned from the sixteen patterns, while the subdivision is an index into a list of twenty-five and the skip chance runs from 0 to 10. A `CLOCK` or a `RANDOM` tab therefore offers nothing to choose, and its `MOD` field **refuses the rotation**. ⚠️ **It still NAMES the routing the channel carries**, because a routing survives a change of mode: a channel routed in `SEQ` and then put into `CLOCK` reads `P/-` and not `OFF`. Showing `OFF` there would be false.

✅ **LOT 13 IS DELIVERED, 2026-09-05: the field is wired and the routing reaches the pins.** The rotation writes both destinations of the channel through `setCvDestination`, and the label derives from them — there is no parallel field, so the screen and the controller cannot disagree. The routing lands in bytes 7 and 8 of the channel record, which lot 10 reserved, so **the format does not move and no version changes**.

**Measured on the pins**, by the `MOD` course of the gesture probe: a channel in `SEQ` plays 16 steps, `1536` ticks, while `CV1` is injected at 4150 mV and nothing is routed. **Two detents on `MOD`** put `CV1` on `LENGTH`, and the same channel plays `2496` ticks, so 26 steps — the base of 16 plus the zone of +10. Two counter-proofs turn it red: one detent routes `PATTERN` instead, and the CV at the module's measured zero leaves the period at 1536.

**Measured on the panel**: the `CONFIG PATTERN` page inks **1015 pixels** with `MOD: P/L`, and the `TypeScript` mirror renders the same number row by row. **Footprint: RAM +6 B, Flash +404 B**, stack unchanged at **216 B**.
...
⚠️ **THE RATCHET FOLLOWS `readStep`, and not `localStep` — decision E3.4-1, 2026-08-31.** The step plays the content of `readStep` and the ratchet of `readStep`. The ratchet is a property of the step (§6.3), so the two come from the same step. To read the content of one step and the ratchet of another produces a hybrid event.
**STEP stays a read shift all the same, and the wider reading is REFUSED.** The channel maps `localStep` to `readStep`, and no content changes. ⚠️ **A consequence to know:** a TRIPLET on `readStep` stretches the duration of the step (§6.3), so the CV STEP can change the progression of the channel in time.
...
**The matrix of the destination by the mode — decided in F0, 2026-09-01.**
<table header-row="true">
<tr>
<td>Destination</td>
<td>SEQ</td>
...
`Y` means the destination acts. `N` means it does not act, and the routing stays.
**Outside `SEQ` the zone of a source routed to PATTERN, LENGTH or STEP holds 0.** A real change of mode puts that zone back to 0. So the derived value returns to its base at once, and the first step after a return to `SEQ` plays the base. ⚠️ **The invariant is this: outside `SEQ`, every zone holds 0.** RESET carries no zone, because it works on an edge (§10.5).
...
The boundary of a **step**, rather than the end of a **pattern**, is a deliberate choice. "The end of the pattern" is not a shared moment: each channel holds its LENGTH and its SUBDIV, and a TRIPLET ratchet **stretches its time** (§6.3), so the ends of the loops drift independently. To wait for the end of the pattern would mean to wait for an unpredictable delay. That delay reaches `MAX_LENGTH` steps, and `MAX_LENGTH` is **36 since lot SF3**, 2026-08-30.
**THE ORDER AT THE STEP BOUNDARY IS A CONTRACT — decision E3.4-4, 2026-08-31.** At each step boundary the channel gives this order:
1. `localStep` advances with the PREVIOUS effective length;
2. the active CV destinations apply;
3. the new `effectiveLength` holds;
4. `localStep` folds if that length requires it;
5. `readStep` comes from the NEW `effectiveLength`;
6. the content AND the ratchet come from `readStep`;
7. the duration and the trigger count of the step come from that ratchet.
⚠️ **The requirement is the OBSERVABLE order, and not a sequence of internal calls.** Any realisation that gives this order satisfies the contract.
**Points 6 and 7 read ONE pattern.** On the boundary where the derived PATTERN index changes (§10.2), the content and the ratchet of the step come from the same pattern. The step never plays the content of the new template with the ratchet of the previous one. The realisation is recorded in ADR 0011.
**Point 5 uses the length of AFTER the boundary — decision E3.4-2.** A CV that takes LENGTH from 12 to 8 gives a `readStep` inside `[0, 7]`. The previous length never serves.
**Point 4 folds at once — decision E3.4-3.** The state "length 6, `localStep` 10" must never exist. ⚠️ The fold happens **only** when `localStep` falls out of range: §7 forbids an arbitrary jump of the playhead.
**RESET is the exception: it is immediate.** A reset that waits is not a reset. It also puts the step clock of the channel back in phase, which is the hard synchronisation people expect. The channel returns to its local step 0 and is **armed**: its entry onset comes out on the **next tick**, and never in the instant. To fire at once would send a burst of triggers on a jittery input, and it would break §9. What that onset plays is the step the channel reads, `readStep` (§10.2). That step is step 0 when STEP is inactive or its zone is zero. Otherwise it is the step selected by the STEP offset, because a reset does not clear the CV zones.
**The armament is an acquired event, and a change of mode never clears it — decision D80, 2026-09-02.** The mode at the moment of the consumption interprets the onset. The semantics are the same for a CV reset and for a global reset (§8, decision D79).
...
**STEP: the offset is ABSOLUTE, and it holds −15 to +15 per source — decision E3.3, 2026-08-31.** STEP reuses the grid of 31 zones. The two sources **add up before the transformation**, as §10.1 asks. ⚠️ **STEP wraps, and it does not clamp.** LENGTH and PATTERN add and then clamp; STEP adds and then takes the positive modulo of §10.2. Two sources therefore give a sum of −30 to +30, and the modulo absorbs it.
**`+N` always means N steps after `localStep`, whatever the length.** That property is the point of the decision.
⚠️ **The PROPORTIONAL offset is REFUSED, and the reason must survive.** It would cover the whole cycle at any length. But at a constant CV, a change of LENGTH would then move the read position too. LENGTH is itself modulable, and §10.3 applies both at the same step boundary.
**The reach depends on the number of routed sources.** With **one** source, the channel reaches **31** of the 36 positions from any `localStep`. The five it misses are `localStep + 16` to `localStep + 20`, modulo 36. ⚠️ **Those five move with `localStep`**, so they are not a fixed range. With **two** sources on STEP, the sum reaches all 36. The clock reaches every position in both cases.
...
**The length of a template runs from 1 to 36.** The upper bound is the capacity of the `Pattern`, and **not** the interface cap `MAX_LENGTH`. The two quantities stay distinct, and they hold the same value of **36 since lot SF3**, 2026-08-30. On the write, a length out of range is **clamped**. On the load, it is **refused**, and the content already read is not lost.
...
**The navigation glyphs are the glyphs of the ORIGINAL.** A decision of the owner, 2026-08-23: the project takes the shapes of the original for the clock and for Play/Stop. And it **creates** a glyph for the global configuration, a **cogwheel**, because the original has no tab for that page. The original shapes are recoverable: `tools/decode-original-font.py` decodes the `velvetscreen` font of the original firmware glyph by glyph. **The provenance must be kept** — the font is GPLv3, like FlexSeq itself, so the reuse is compatible and only the attribution is due.
...
**4. THE GLYPH OF THE GLOBAL CONFIGURATION IS NOT A COGWHEEL, and this SUPERSEDES the sentence above.** This section says the project *creates a cogwheel*. A cogwheel at 5 x 5 is not readable, which the owner judged on the mockup. The chosen shape is **two sliders**, 5 x 5, and it is created by us. The glyph keeps the size of its neighbours in the bar, which are all 5 x 5. ⚠️ **THE SIZE IS 7 px BY 5 px SINCE 2026-09-09, and the sentence above says 5 by 5.** The owner chose the size on the render of the pixel mirror. At 5 px wide the knob of one slider touches the other slider, and the two read as one mark. ⚠️ **And the claim that every glyph of the bar is 5 by 5 was already FALSE.** §5.0 point 6 gives the grid of `PATTERNS` 7 px by 5 px, and the code drew it 7 px wide. A glyph of 7 px by 5 px therefore matches its neighbour, and the slot of the bar holds 12 px. ⚠️ **THE GRID OF `PATTERNS` IS REMOVED since 2026-09-14**, so the `CONF` glyph is now the only wide one. The size it took from that neighbour does not move.
...
**10. `MODE` IS THE FIRST FIELD IN THE THREE MODES, and it is what makes the
mode reversible.** Decided 2026-09-04, during lot 11. The original places `MODE`
on the first of its three lines whatever the mode, and FlexSeq does the same. In
`CLOCK` and in `RANDOM` the tab holds three fields, so the field list **is** the
three lines of the original. In `SEQ` the tab holds six, and `MODE` takes index
0 ahead of `PATTERN`.

⚠️ **Without `MODE` in the `SEQ` list, the mode would be a ONE-WAY TRIP.** A
channel that enters `SEQ` would hold six fields, none of which changes the mode,
so nothing could bring it back to `CLOCK` or to `RANDOM`. That was a real defect
...
⚠️ **THE TITLE IS FLUSH LEFT SINCE 2026-09-09, at `x = 4`.** It was centred, so it floated between the edge of the screen and `SEP`. It now sits on the column of the rule that underlines it. And **`SEP` carries its colon**, `SEP:`, like every other label of the interface. Measured in `velvetscreen`: `SEP:` is 16 px and `SEP` is 14 px, so the label grows by 2 px. The title keeps its 65 px, and it now ends at `x = 68` against the label at `x = 102`. ⚠️ **AND THE FRAME OF THE OPEN VALUE CLEARS IT BY 2 px ON EACH SIDE, since 2026-09-13.** It cleared it by 1 px, so the frame touched the digit on all four sides and the two read as one mark. The owner read that on the module. `SEP_FRAME_PAD` is 2, the frame runs from `x = 118` to `x = 125`, and the columns 119 and 124 stay empty beside the digit. **Measured: RAM +0 bytes, Flash +0 bytes.** ⚠️ **The frame does NOT grow upward or downward**, and a measurement says why: a clearance of 1 px above and below needs 9 rows, and the band holds 8. The header must stay in band 0, which two `static_assert` hold.
...
- **Wokwi** (a VS Code extension: `wokwi.toml` and `diagram.json` at the root): a **VISUAL validation of the OLED render**. Wokwi runs the **real firmware** that PlatformIO compiles (`pio run -e wokwi` → `src/wokwi_main.cpp`): the same domain, the same renderer, and the same `gravity.display` object. **No copy of the code**, so no drift is possible. `env:wokwi` is the real firmware plus a preloaded demonstration content, and `main.cpp` stays empty, on the same principle as `simavr_main.cpp` for the signals.
	**The SCHEMATIC handles the rotation**: `diagram.json` applies `"rotate": 180` to the screen, which models the physical mounting, an OLED upside down. The firmware therefore keeps its `U8G2_R2` **with no divergence at all**. The OLED sits above the board: turned, its pins go to the bottom and face the Nano, so the wires do not cross the screen.
	...
	· **On the firmware side: VERIFIED.** `tools/run-screen-dump.sh` reads the memory of the SSD1306 panel inside simavr. It **asserts** that the image there is turned by 180°, with the title at the bottom and the top of the panel empty. It also asserts that the full set of grid positions defined by `LENGTH` sits at the expected place after `U8G2_R2`. That is no longer a piece of reasoning but a test: a change of the rotation constant would turn it red.
...
**Current probe snapshot — 2026-09-03, commit `f29cba7`, the STEP-11 acquittal.** Trigger probe: **11 courses** (clock, seq, ratchet, cvzero, cv1length, cv2length, cvreset, patold, patnew, cvpattern, cvstep), 54 s. Tests: **631** C++ assertions, **12** adapter assertions, **580** TypeScript tests, the typecheck clean, the image check **24/24**, the libGravity conformity **7/7** audited anomalies. Mutation: **326/326** on the complete pass, 328 declared. Main loop: **p90 6.91 ms**, median 4.82 ms, identical on the baseline commit, so no measurable regression. `nanoatmega328`: **28764 B** of Flash and **1512 B** of RAM. Stack peak **207 B** on four courses, the STEP course included. The figures below are the snapshot of 2026-08-31, kept as dated records.
...
⚠️ **Two entries of the list below are SET ASIDE since 2026-09-07, and a third is DEFERRED**, by the amendment of §5.0. Set aside, and out of the specification: the four modes with **GATE** · **SWING** as a parameter of `SEQ`. Deferred, and still a requirement: the **mute** on `SHIFT` plus `PLAY` — only its delivery waits. All three keep their room reserved in the record of a channel. Nothing else in the list moves.
...
4. **The acceleration of the encoder disappears everywhere, the tempo included.** The original does not accelerate, so FlexSeq will no longer hold a rule and an exception, but one rule. **An accepted consequence**: the tempo range holds **280** values, so to cross it asks for 280 detents. ⚠️ **This figure said 270 until 2026-09-03**, when the range became `[20, 300]`; see decision 8 below.
...
8. **The tempo range becomes `[20, 300]` — decided on 2026-09-03.** The interface accepted `[30, 300]`, and the original firmware accepts `[20, 200]`. The owner lowered the minimum to the original's 20 and **kept the maximum at 300**. Keeping 300 is a **deliberate divergence**, of the same class as decision 1 above: the four external clock rates against the original's two, an addition that removes nothing. ⚠️ **The choice makes a persistence question disappear.** A range of `[20, 200]` would have put every stored tempo between 201 and 300 outside the bounds, with no rule for the load. `[20, 300]` **contains** the previous range, so no stored value can fall outside it. The clamp to the nearest bound is implemented all the same, as a net for a future narrowing. Measured on the pins at 20 BPM: a step of **2999.86 ms** against 3000.00 expected, and a jitter of 0.29 ms, so 0.01 % of its budget. ⚠️ **No warning is shown when a value is clamped**, and that is a choice rather than an oversight: the main screen carries no free space, and the cost was judged higher than the benefit.
