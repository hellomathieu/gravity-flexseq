...
**A global reset ARMS the entry onset of step 0 — decision D79, 2026-09-02.** `start()` and `reset()` put every channel on step 0 and arm it. The first tick after the reset emits the entry onset of step 0. The trigger then follows the predicate of the mode at that moment (§4.2, §9). `resume()` arms nothing. This is the behaviour of the original: `resetClocks()` puts the cursor to 0, and the first `clock()` serves the current step before the increment. Before this decision the first onset belonged to step 1, and the armed mechanism of §10.3 carries the alignment.
