...
---
## 1. Vision
Gravity FlexSeq est un firmware alternatif du Gravity, développé **sans modification du hardware** et basé techniquement sur `libGravity`. Il fait évoluer le **Trigger Sequencer** d'origine (16 steps fixes) vers un séquenceur dont les patterns ont une **capacité de 36 steps**, la longueur réglable à l'interface allant de **1 à ****`MAX_LENGTH`** — **36 depuis le lot SF3, 2026-08-30** —, avec une conception temporelle plus générale, tout en conservant les fonctionnalités historiques du module.
⚠️ **Ce paragraphe et le sous-titre annonçaient `MAX_LENGTH` à 24 jusqu'au 2026-08-31, et ils attribuaient le plafond de 36 au lot F.** Les deux étaient périmés : le lot **SF3** a porté `MAX_LENGTH` de 24 à 36 le 2026-08-30, et le lot **F** a porté la grille d'édition à 36 positions le même jour. Le compte complet vit en §5.2 et en §12.1, et ce paragraphe ne le recopie pas.
