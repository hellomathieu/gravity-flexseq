...
**A global reset ARMS the entry onset of step 0 — decision D79, 2026-09-02.** `start()` and `reset()` put every channel on its local step 0 and arm it. The first tick after the reset emits the entry onset of that local step. Its content and its ratchet are those of `readStep` (§10.2): step 0 when STEP is inactive or its zone is zero; otherwise the step selected by the STEP offset. The trigger then follows the predicate of the mode at that moment (§4.2, §9). `resume()` arms nothing. This is the behaviour of the original: `resetClocks()` puts the cursor to 0, and the first `clock()` serves the current step before the increment. Before this decision the first onset belonged to step 1, and the armed mechanism of §10.3 carries the alignment.
...
⚠️ **THE MUTUAL EXCLUSION IS LIFTED — decided by the owner on 2026-08-22.** A channel can route **CV1 AND CV2 at the same time**, each one to its own destination. Two modulations that target the **same** destination **add up and then clamp** — STEP wraps instead (§10.4) — which is a direct generalisation of the "additive offset and then clamp" below.
...
**Points 6 and 7 read ONE pattern.** On the boundary where the derived PATTERN index changes (§10.2), the content and the ratchet of the step come from the same pattern: never the content of the new template with the ratchet of the previous one. The realisation is recorded in ADR 0011.
