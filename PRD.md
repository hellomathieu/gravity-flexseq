...
>
> **The user state lives entirely in the EEPROM, and FlexSeq writes nothing there.** The original firmware stores the bpm, the master clock mode, the channels, the **16 sequences A1–B8**, the CV calibration and the rotation preference. FlexSeq has **no persistence yet** (§11), so it writes no byte of EEPROM, and an upload through the bootloader does not touch it. Make the backup before the first flash anyway: "it should survive" and "it did survive" are not the same statement.
>
> ✅ **Backup of the Flash: DONE and VERIFIED on 2026-08-21.** 32768 bytes, an authentic interrupt vector table at the head, 14.8 % of `0xFF`. The project keeps it **outside the repository**: it is the GPLv3 firmware of the manufacturer plus the settings of the owner, and to version it would publish it. The restoration covers the **application region**. The bootloader itself cannot rewrite the bootloader region, and that region is never at risk, because none of our operations reaches it. **Not tested**: to verify that we can restore would require us to overwrite the original firmware.
>
...
>
> ⚠️ **Consequence for §10: the real CV calibration is out of reach for now.** libGravity holds **no** notion of EEPROM, and that is verified on the source of the pinned commit. `CvSampler` therefore takes its configuration from the defaults of libGravity, `CALIBRATED_LOW = -566` and `CALIBRATED_HIGH = 512`, and not from the calibration of this module. That calibration exists only in the EEPROM, and in a layout that only the original firmware knows. Tracked: `docs/open-risks.md` line 17.
---
...
**8. Grille de 36 steps à l'écran — IMPLÉMENTÉE le 2026-08-30, lot F.** 3 lignes de 12 pleines, donc 36 emplacements et aucun mort. Le pas de colonne et **la taille des glyphes ne changent pas** — la lisibilité passe avant. **Le pied disparaît de l'écran EDIT**, comme dans l'original. Centres des rangées : **18, 36 et 54**, espacement de 18 px inchangé, dernier pixel de la grille à **63**.
⚠️ **Deux leviers verticaux avaient été identifiés pour dégager la place** : remonter la ligne de titre, et réduire l'espace entre le glyphe et son chiffre de ratchet. **La géométrie retenue n'a eu besoin d'aucun des deux** : la suppression du pied suffit. Le titre garde sa ligne de base à 7, donc sa bande unique, et `DIGIT_DY` reste à 5. Les deux leviers restent disponibles pour un besoin futur ; ils ne décrivent pas l'implémentation.
...
- `1 ≤ effectiveLength ≤ MAX_LENGTH` par channel — **`MAX_LENGTH` vaut 36 depuis le lot SF3, 2026-08-30**, la capacité du `Pattern`. ⚠️ **La capacité de stockage et le plafond d'interface restent deux quantités distinctes, et elles portent maintenant la même valeur.** Les deux bornes ont convergé à 36 ; elles ne se lisent pas l'une l'autre, et les deux points d'entrée restent séparés — voir l'amendement d'ADR 0009. Longueur **déduite du template au chargement** (dernier pas non vide) — ⚠️ **et cette déduction ne sert plus qu'à un slot VIDE qui reçoit un contenu pour la première fois.** Partout ailleurs la longueur vient de l'enregistrement du modèle, qui la STOCKE : 24 octets, 23 de contenu plus 1 de longueur. Voir le §5.0 point 3, qui fait foi. Elle est ensuite éditable dans le channel.
- Une modulation CV peut produire une `effectiveLength` temporaire **dans les deux sens** — raccourcir **ou** allonger — clampée à `[1, MAX_LENGTH]`, soit `[1, 36]` depuis le lot SF3 (**décidé 2026-08-20, borne liée au plafond d'interface le 2026-08-28, plafond porté à 36 le 2026-08-30**). La valeur réglée à l'écran et persistée est la **base** ; le CV ne l'écrase jamais. Voir §10.
...
- **LENGTH est GELÉE pendant un RECORDING, Length CV compris — décidé le 2026-08-30 (lot LCV).** La longueur active au démarrage de l'enregistrement est celle qui structure le pattern enregistré ; la changer en cours de route déplacerait les steps déjà écrits. Le CV **continue d'être échantillonné** — la chaîne d'acquisition sous interruption (§10.6) n'a aucun état à geler — mais son offset **n'est pas appliqué** tant que le RECORDING dure. À la sortie, la modulation reprend **à la prochaine frontière de step**, par la règle générale du §10.3. ⚠️ **Le mode RECORDING n'existe pas dans le code** : cette décision borne sa conception, elle ne l'anticipe pas. Le lot qui l'implémentera ajoutera la garde **au point unique d'application du CV**, sans créer de seconde voie.
...
- **LENGTH** — `effectiveLength = clamp(base + f(cv), 1, MAX_LENGTH)`, à la frontière de step. `MAX_LENGTH` vaut **24 jusqu'au lot SF3**, puis 36.
