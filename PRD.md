...
⚠️ **La grille et la capacité du ****`Pattern`**** valent maintenant 36, mais l'EXÉCUTION en vaut 24.** `screen::GRID_STEPS` et `UiController::STEP_COUNT` valent **36** depuis le lot F ; `SequencerEngine::MAX_LENGTH` vaut **24** jusqu'au lot SF3. Les steps 24 à 35 sont donc **visibles, éditables et persistés, mais jamais joués** : l'écran les dessine comme des points isolés, la représentation d'un step au-delà de la longueur. Quatre mutants tiennent cette séparation dans les deux sens, et aucune de ces trois constantes ne lit les autres.
>
